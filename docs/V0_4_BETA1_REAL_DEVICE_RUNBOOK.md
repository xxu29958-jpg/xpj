...
2. 在 `成员` 表可把非 owner 成员在“成员（可写）”和“只读”之间调整；角色会在该成员下一次 API 请求时生效。
3. 在 `成员` 表对该 Member 点击“禁用”：该成员所有 AuthToken 同步撤销，下一次 API 请求 401。
4. 在 `邀请` 表对未使用的邀请点击“撤销”：邀请立刻不可接受（即使在 TTL 内）。
5. 已接受过的邀请不会撤销已建立的成员；要切断已加入的成员请用第 3 步。
...
- Owner 转让暂未在 Owner Console 暴露 UI；member/viewer 角色调整已支持。
