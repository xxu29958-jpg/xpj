...
* `client_request_id` 幂等键（vs 模板 §3）：当前写操作不带 client-side dedup key；上传/创建幂等由服务端业务键（如 image hash, idempotent confirm）保证。
* `/health/liveness` + `/health/readiness` 拆分（vs 模板 §12）：当前只有单一 `GET /api/health`（loopback 公开，Cloudflare Tunnel 暴露），不区分进程存活与依赖就绪；v1.0 起单机 + Windows 任务计划部署模式下没有编排器去消费 readiness 探针。
* UI 字符串走资源文件（vs 模板 §10 "UI 字符串必须走资源文件，预留 i18n 通道"）：Android Compose 当前在 30+ 文件里直接 `Text("中文字面量")`，`strings.xml` 仅保留 `app_name`。与模板 §13 "当前阶段不做：完整 i18n / 完整 a11y" 一致——本项目不准备做翻译，也不预留部分资源化通道；要重新引入必须先开 ADR，因为是 30+ 文件批量重构。
