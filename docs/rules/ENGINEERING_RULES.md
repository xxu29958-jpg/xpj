...
> §14 在 2026-05-22 增"字段命名"+"当前阶段不引入"两小节：解释 `expense_time`/`tenant_id` 项目命名与模板 §3/§4 的关系，并显式标注当前 v0.x 不引入 `/api/v1` 前缀和 `client_request_id` 幂等键。
...
以下是与模板 §4/§3 形式不一致但 v0.x 暂不引入的项；要引入必须先开 ADR：

* `/api/v1/...` 显式版本前缀：当前所有 API 是无版本 `/api/*`，破坏性变更直接 bump 后端 `BACKEND_VERSION` + 同步 Android DTO + OpenAPI snapshot；v1.0 启动时再 ADR 决策是否引入。
* `client_request_id` 幂等键：当前写操作不带 client-side dedup key；上传/创建幂等由服务端业务键（如 image hash, idempotent confirm）保证。
