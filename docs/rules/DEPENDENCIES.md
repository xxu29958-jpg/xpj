...
- 来源/安装：官方下载页 `https://jrsoftware.org/isdl.php`；也可按官方命令 `winget install --id JRSoftware.InnoSetup -e -s winget -i` 交互安装。构建脚本优先找 PATH，再找 `C:\Program Files (x86)\Inno Setup 6\ISCC.exe` / `C:\Program Files\Inno Setup 6\ISCC.exe`。
