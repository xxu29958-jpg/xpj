...
- 项目 pin：**Inno Setup 6.7.1**。截至 2026-08-24，官方当前稳定版是 **7.1.0**；本切面只记录该候选，不升级已冻结的 compiler identity，升级必须单独更新工具链合同、SHA、provenance 与安装器运行证据。
- 来源/准备：唯一声明位于 `backend/packaging/windows-build-toolchain.json`；运行 `backend\packaging\prepare_windows_build_toolchain.ps1 -Component Inno` 下载官方固定归档、先校验归档 SHA，再发布并复核 `backend\build\windows-toolchain\inno\ISCC.exe`（SHA-256 `eb6f4410c8db367a5f74127e8025ad2ccacc0afabbe783959d237df3050f97fb`）。构建和测试都不搜索 PATH、`LOCALAPPDATA` 或 Program Files。显式 `-InnoCompiler` 也必须与合同声明的 engine version 和 executable SHA 完全一致，否则拒绝构建。
- 简体中文语言文件：仓库内置 `backend/packaging/languages/ChineseSimplified.isl`，来源 JRSoftware `https://jrsoftware.org/files/istrans/` 指向的 `https://raw.githubusercontent.com/jrsoftware/issrc/refs/heads/main/Files/Languages/ChineseSimplified.isl`；上游 Git blob SHA `0fced9759dd20e250101768985af511153133cdc`。本仓库在翻译文件上叠加 `Microsoft YaHei UI` 的 `DialogFontName` / `WelcomeFontName`，避免 en-US UI 的干净 Windows 把简中向导绑定到无中文字形的 `Segoe UI`；当前规范化 LF 文件 sha256 `c7f406b81d1a5178fc07f9245159fab48fb8ceea1aa8a899192e0a4890345169`，适配 Inno `6.5.0+`。更新语言文件或字体叠加时必须同步本条 hash。
- 许可证：**Inno Setup License**（官方称 open-source；商业使用官方建议购买商业 license，项目分发前按实际用途处理）。构建脚本只依赖 `ISCC.exe`，不改运行时依赖树。
- 验证要求：`packaging\build_inno_installer.ps1 -CheckSourceInputsOnly` 只验证受版本控制的源码/配置，不能冒充 Inno 或安装包资格；完整构建必须使用上述 pinned compiler，实际生成 `dist\installer\Ticketbox-Setup-<version>\` 原子发布单元，并在干净 Windows VM 验证安装、启动、恢复、重启、升级与卸载生命周期。
