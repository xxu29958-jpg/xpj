...
> **历史工程排期（2026-05）**：保留当时 v0.5→v1.0 的规划与实现背景，已不拥有当前目标、版本顺序或验收门的裁决权。
> 当前依据为有效 Goal、Gmail 最终合同及后续明确裁决；现状与施工顺序见[当前产品地图与修订规划](../current/TICKETBOX_CURRENT_PRODUCT_ATLAS.md)。已有能力继续保留并加强。

日期：2026-05-12（最后更新：2026-05-16）

本文记录当时 `v0.4-beta1` 之后到 `v1.0` 的开发路线。下文的“当前”、完成状态与版本门均指当时基线，不能用来描述现在的 `main` 或缩小完整产品目标。

同目录 `MONARCH_CAPABILITY_ROADMAP.md` 保留当时的产品能力对照；二者都作为历史参考，当前规划统一由上方产品地图导航。
