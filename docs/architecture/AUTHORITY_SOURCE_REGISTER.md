...
| AUTH-RBAC-001 | 当前 ledger capability 来自有效 app/Web principal + active `LedgerMember.role`；admin/upload/recovery 不是 member role | `permission_service` + route/service ledger scope；[[0068]] | loopback、Host、Cloudflare Access、OS admin 或隐藏按钮不能替代应用授权 | **nonconformant**：Owner Console 隐式 owner 待迁移 |
