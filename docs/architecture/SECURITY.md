...
- Windows 备份脚本使用 SQLite Online Backup API 将 `<DATA_ROOT>\data\ticketbox.db` 快照到 `<DATA_ROOT>\backups`(源码运行 `backend\`,冻结 EXE `ticketbox-data\`),清理旧备份前会校验目标仍在备份目录内。
- 数据库恢复脚本只允许从 `<DATA_ROOT>\backups\ticketbox-*.db` 恢复，覆盖前会使用 SQLite Online Backup API 自动备份当前数据库。
