...
- 统计页增加“月预算”进度；预算保存在本机设置，只用于用户自我提醒，不改变服务端统计口径。
- 设置页自动分类规则拆成独立底部管理面，主设置页只保留启用数量摘要和管理入口。

## 后续 P1

- 设置页后续只保留必要摘要，继续避免把维护信息堆成后台控制台。
- 统计页已补充分类洞察，隐藏 0 数据，只呈现主要分类占比、集中/分散状态和均笔金额。
