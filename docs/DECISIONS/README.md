...
**编号范围**：0001–0065（0018 保留 rejected tombstone；0032–0034 为保留号，不得复用）。
...
| # | 标题 | 一句话 | 状态 | 关系 |
|---|---|---|---|---|
| [0001](0001-money-uses-cents.md) | 金额使用分保存 | 历史决定：整数金额、禁 float；当前 minor-unit 语义见 0061 | accepted / implemented / unverified | — |
...
| [0016](0016-performance-and-stability-baseline.md) | 性能与稳定性基线 | SQL 层聚合 + Retrofit 复用 + 批量 upsert + 限并发缩略图 | superseded / implemented / stale | — |
...
| [0022](0022-family-ledger-permission-model.md) | 家庭账本权限模型 | owner / member / viewer 三态 + 邀请 token；账户隔离不破 | superseded / implemented / stale | — |
| [0023](0023-chart-library-policy.md) | 图表库引入政策 | 阶段化：v0.9 经审计后可引入展示层图表库 | accepted / implemented / unverified | — |
| [0024](0024-tri-surface-ui-ux-unification.md) | 三端 UI/UX 美化统一 | 统一设计语言，不强行同屏复刻 | accepted / partial / unverified | — |
...
| [0027](0027-backend-authoritative-fx.md) | Backend Authoritative FX | 后端唯一汇率权威；冻结快照；缺率返 pending | accepted / implemented / unverified | — |
...
| [0038](0038-v1.3-multi-surface-sync.md) | v1.3 Multi-Surface Sync | row_version OCC + Android outbox + 显式冲突；当前扩展见后续 ADR | accepted / nonconformant / failed | — |
...
| [0042](0042-offline-availability-and-request-idempotency.md) | 离线可用性边界 + 请求幂等键 | outbox-routed mutate 使用 intent-time key，分离 OCC 与安全 replay | accepted / nonconformant / failed | amends 0038 |
...
| [0045](0045-csrf-signing-key.md) | CSRF 持久化随机签名密钥 | 公开占位常量 → app_meta 随机秘密，启动 fail closed | accepted / partial / unverified | — |
| [0046](0046-android-recurring-reminder-detection-source.md) | Android 固定支出提醒检测源 | WorkManager 只作 Scheduler；Engine/Policy/Store/Dispatcher 分层 | accepted / partial / unverified | — |
| [0047](0047-bundled-installer-windows-services.md) | 捆绑安装器 + Windows 服务化 + 主机管理器 | Windows 服务化与家庭分发方向；实施状态单独跟踪 | accepted / partial / stale | depends-on 0041; depends-on 0028; depends-on 0045 |
| [0048](0048-rive-mascot-animation.md) | Rive 吉祥物动画方案（已放弃） | Rive 导出付费墙导致方案撤回；当前使用原生 Compose | rejected / not-started / stale | — |
| [0049](0049-debt-domain-contract.md) | Debt Domain Contract | frozen obligation + append-only repayment/correction/forgiveness facts | accepted / partial / unverified | depends-on 0001; depends-on 0068; depends-on 0027; depends-on 0029; depends-on 0038; depends-on 0041; depends-on 0042 |
...
| [0062](0062-windows-installer-lifecycle-transaction.md) | Windows 安装事务：生命周期回执、复制边界与恢复协议 | 用机器锁、持久回执和复制边界区分可补偿失败与必须 repair 的故障隔离 | accepted / nonconformant / failed | amends 0047; depends-on 0041; informational 0006 |
| [0063](0063-recoverable-owner-bootstrap-ceremony.md) | 可恢复 Owner Bootstrap：确定性凭据、全局串行化与关闭窗口 | 以家庭 owner 身份 claim 为核心，隔离可恢复创建、可撤销子凭证和安装交接 | accepted / partial / failed | amends 0047; depends-on 0028; depends-on 0045 |
