...
1. Windows 本地脚本 `scripts\real_device_preflight.ps1` 检查后端、Token、测试上传和 Android 设备安装。
...
把预检做成脚本可以减少手工排错；把 App 内自检保留在设置页，可以在真机、Cloudflare 域名和实际 Token 下重复验证。

## 约束

- 预检脚本可以读取 `backend\.env`，但不得打印 Token。
