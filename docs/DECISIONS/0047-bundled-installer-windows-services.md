...
1. **双服务，SCM 当守护 + 优雅关停**：安装器（提权一次）注册 PG 服务 + 后端服务，后端用 **Shawl** 包 PyInstaller EXE（Rust 单文件、维护中，专为「子进程响应 ctrl-C/SIGINT」设计 = uvicorn 最佳契合；**NSSM 否决——末版 2017 弃维违 §9**；WinSW v2 作 .NET 兜底）。依赖排序：**`sc config <后端服务> depend= <PG服务>`**（Shawl 自身不设依赖，须 sc.exe 配）+ `start= delayed-auto` 避早启动竞争 + `sc failure` 重启退避（如 5s/10s/60s，reset 3600s）。**优雅关停**：Shawl 停服务时向子进程发 **Ctrl-C**（uvicorn 收 SIGINT → 干净 lifespan shutdown）；Shawl `--stop-timeout` 默认仅 **3000ms（太短）→ 调大（如 25000ms）**并让 uvicorn 设 `timeout_graceful_shutdown` 匹配，超时才强杀。
...
* **graceful-shutdown 不变量**：服务停产生干净 uvicorn lifespan shutdown（日志可见），非 TerminateProcess 硬杀（`--stop-timeout` 调大 + uvicorn `timeout_graceful_shutdown` + Shawl ctrl-C）；达不成则记为「PG-only 可容忍的已知 fallback」，不得是静默默认。
