...
The reviewer confirmed the two follow-ups on `65cf3583` and closed the P2. That candidate's cloud CodeQL and both ordinary PostgreSQL shards passed; Android fast reported Detekt `DebtDraftForm` length 73 against the existing 60-line limit. FIX: extract only the new note input into `DebtContextField`, keeping state values, callback and enabled condition identical. Do not alter the baseline or add a new behavior test for an equivalent component extraction. Final cloud qualification remains open; earlier candidate results cannot qualify the next head.

The same candidate's real-db 1/2 (`101301989781`) reproduced three managed-generation failures: after Alembic reached the new target, the executor revalidated the historical Expense-offset postcondition, which incorrectly required its own revision to remain the head. FIX: that postcondition and the new Debt postcondition distinguish their inline upgrade checkpoint (Alembic still names the parent) from subsequent exact live-head revalidation. Each requires a single version row and exact authority equality for that phase; no invariant, transition, transaction, admission or writer fence is removed. Existing real generation/CAS/read-only consumer tests are the failing regression, so no duplicate harness is added. The candidate's Connected execution, installer build and remaining PostgreSQL shards passed, but the final next-head cloud run must independently qualify all applicable lanes.

Candidate `8b96b7dc`, CI `33965293044`, passed all PostgreSQL shards including the three generation regressions, contracts, installer and APK builds. CodeQL `33965292957` and Connected `33965292966` passed. Android fast compiled and ran unit tests, then failed the existing per-file function limit: adding the field helper made `DebtListScreen.kt` contain 12 functions against 11. FIX: move the existing creation sheet, its form and its two private fields unchanged into cohesive `DebtCreateSheet.kt`; the list still invokes the same sheet, with the same state, actions, authority checks and failure retention. This is a file-responsibility move, not a baseline increase or behavior change. Exact final-head cloud qualification remains required.
