...
iOS 26.4 真机验证通过（验证范围 = 动作 1–3 的上传链路与原先的静态「显示通知」收尾；下面动作 4–5 的疑似重复分支通知是后续增补，**待真机回归**）。**关键约束**：动作 2 的「URL」字段必须粘贴 Owner Console 给出的<u>完整公网 URL</u>（以 `https://` 开头），iPhone 才会把它当成有效 URL；只复制 `/u/...` 相对路径或 `/u/***` 掩码会被 iPhone 直接判定为无效 URL。
...
4. **动作 3：获取 URL 内容**：
   ...
   - 文件：选择「转换后的图像」
5. **动作 4：获取词典值**，从上一步「URL 内容」取键 `duplicate_status`。
6. **动作 5：如果**，条件「词典值 等于 `suspected`」→ 显示通知「已上传，但疑似重复票，记得去待确认里看一眼」；「否则」→ 显示通知「已上传到小票夹 ✓」。（该分支待真机回归。）
...
`duplicate_status` 在响应返回前同步算好：`none` = 没发现重复，`suspected` = 与已有小票疑似重复（此时 `duplicate_of_id` 指向疑似原票），快捷指令可直接据此分支通知。
...
- 上传失败时（4xx）后端返回的同样是 `{"error":"...","message":"..."}` JSON 信封，例如 401 `invalid_token`（链接失效）、429 `upload_throttled` / `upload_daily_quota_exhausted`（限流/日配额），此时响应里没有 `duplicate_status` 键。iOS 快捷指令对非 2xx 响应的具体表现（中断报错、还是继续走「否则」分支发成功通知）**待真机实测**，排查时以后端日志和待确认列表为准。
