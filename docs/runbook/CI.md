...
- GitHub: push 到 `main`，以及 pull_request 到 `main`。PR 始终跑 Backend 快速合同；PostgreSQL、Desktop、Android、Windows packaging 只在对应路径受影响时跑。Desktop BFF 允许的 `/static/web/`、`/static/shared/` 以及 `/web` 产品面（`backend/app/templates/web/`、`backend/app/routes/web_*` 与 `_web_*`，不含被 BFF 拒绝的 `web_auth.py`）同时选中 PostgreSQL、冻结 Backend 与 Desktop。主 CI、CodeQL Android 和 Connected 复用同一 `ci_scope.py` 分类权威；`main`、完整资格、未知路径、空 diff 或分类失败均 fail closed 到全量。required 检查名 `CodeQL` 由 workflow 内始终运行的同名 job（GitHub Actions）在 exact SHA 上发布；Code Scanning 的 Analyze job 绿灯不能代替这个 context。
...
`Windows native contracts` 复用原有 coordinator job，在独立 Windows runner 上准备同一份钉住版本的 vendor 输入，再串行执行完整 `packaging/tests -m xdist_group` 集合。安装包构建 job 保留互补的 `not xdist_group` 四 worker 测试、真实 Desktop/BFF/配对、构建及原子产物回验；两组由同一 marker 自动选择，新测试不依赖人工名单。现有 `Windows release packaging` 仍要求两个 job 成功并核对各自 checkout/source 身份，测试失败不能借构建成功取得资格。没有新增 job 或改变 timeout；额外 vendor 准备与缓存开销计入完整运行成本。
