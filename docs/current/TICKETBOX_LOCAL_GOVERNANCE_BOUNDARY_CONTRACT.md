...
| `maintenance_ticketbox.ps1` / `DATA_RETENTION.md`；`smoke_test.py` | 维护脚本默认 loopback，但 ServerUrl 参数未说明支持边界；烟测自行启动本机后端 | 现有帮助/说明明确源码或测试环境、本机环回地址和可选本机端口；参数和调用机制不变；PowerShell parser 与实际 `Get-Help -Parameter ServerUrl` 通过。没有执行维护或烟测 |
