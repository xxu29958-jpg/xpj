...
- **PR-2g.11 Android 离线同步界面（conflict UI，ADR-0038 键石）**：
  - ADR-0038 要求"409 → conflict UI 状态机，让用户选覆盖远端 / 丢弃我的"。数据层早就绪（`OutboxRepository.observeStatus` / `resolveConflict(KeepMine/DropMine)` / `resolveFailed(Retry/Drop)`），但**一直没有 UI 消费**——dispatcher 产出的 CONFLICT / FAILED row 用户看不见也没法处理，ADR 不变量对用户没兑现。本 PR 补上。
  - **新 `OutboxStatusViewModel` + `SyncStatusScreen`**（设置 → 离线同步）。VM observe `observeStatus()`，暴露 keepMine / dropMine / retry / dropFailed。keepMine 需要服务端最新 token（409 body 不含，ADR §41）→ 重 GET 资源；v1 支持 expense 族（`expense:<id>`，PR-2g.3–.9 全部 dispatcher 的 target），其余族 drop-only（rule/alias keepMine 留后续）。
  - **设计按统一骨架 + design token**（不照抄旧屏）：`SettingsPageFrame` + `SettingsSection` + `AppSolidCard` + `LocalStateTokens`。一张焦点状态卡，三态**冷静不报警**——全部已同步（success）/ N 笔待同步（info，不是红）/ N 笔需要你处理（danger）；冲突/失败行用人话（"离线时：确认入账"）+ 每行一个主操作（用我的覆盖 / 重试）+ danger outlined（放弃）。
  - **接线避开 god object**：走自包含范式（同 BackgroundTasks/BillSplits）——Host 分支里 `viewModel(factory=outboxStatusViewModelFactory(outbox, expense))` 自建 VM，只要 `onBack`，**零回调进 30-callback 的 `SettingsRouteActions`**。仅 `MainScreenFactory` / `SettingsRouteRepositories` 各加 1 个 `outboxRepository` 字段，沿现有 repo 穿线。
  - **不在此 PR**：rule/alias 族的 keepMine token 重取；OutboxStatusViewModel 单测（resolve 逻辑薄、只委托给已测的 `OutboxRepository.resolveConflict/resolveFailed`，留干净后续）；全局 queue-depth pill（本 PR 入口在设置）。
