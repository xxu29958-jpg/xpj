...
| Budget aggregation and remaining consumers | Confirmed expense/refund streams carry recorded currency; budget and income projections share the existing FX owner. Missing conversion stays unknown. Recurring totals now use captured currencies in the candidate implementation. Period reports, spending goals and advisor spending readers still require common-currency projection. Monthly-limit saves no longer evict advice because saved limits are not advisor inputs. Nonempty default changes remain unavailable until the affected consumers close |
| Recurring money (in construction) | Backend/Web candidate: required captured currency on create/edit/candidate confirmation; edits cannot relabel amounts. Existing idempotency rows retain accepted manual command receipts instead of replaying a later revision. Migration/adoption preserve money, versions and relationships. Candidates, anomalies, budget/advisor reservations and the Web overview reuse FX projection; missing inputs stay unavailable. Item/editor, payment and recycle views use recorded units. Incompatible raw drafts retain their original and offer a separate current editor. Owner reminders consume dates/counts only. Android candidate now persists manual commands first, retires direct HTTP and the Synced outcome, preserves the original OCC token, checks accepted receipts and retains unsupported rows. Completed-row observation refreshes facts even if the pending emission was skipped. DTOs and editor sessions capture the original currency; a missing or incompatible currency cannot submit or rebase. Backend migration/API/Web cloud groups passed. Android global recovery and record/list/Plan money displays are wired candidates. Occurrence payment capture/display remains open; current recovery/display changes await cloud qualification. Direct producers: recurring currency/receipt/projection and migration tests, Web/occurrence/recycle/adoption suites, Android DTO/dispatcher/repository/recovery/UI tests. No deployment or completion claim |
| Recurring Android continuation | Entries: RecurringScreen, Plan overview, obligation sync and Settings sync. RecurringRepository owns original admission/description/recovery; the existing Outbox owns persistence and dispatch. Global default formatting, raw mixed sums, blind retry/drop and unused current-display editor arguments are retired in the candidate. Preserve keys, payload currency/OCC, original amounts and binding; unsupported legacy rows remain readable. Both navigation factories, pending/drop summaries, item/candidate/Plan views and direct unit/Connected producers migrate together. Cross-ledger admission, unsupported retry, mixed totals and saved-state tests bound this construction; occurrence continuation is still open. No second FX or recovery owner. |
