...
| Budget aggregation and remaining consumers | Confirmed expense/refund streams now carry recorded currency; budget projections and income share a read-only FX helper. Missing conversion stays unknown. Period reports, spending goals and advisor spending readers still need common-currency projection; monthly-limit saves no longer evict advice because saved limits are not advisor inputs; the actual drain callback has a direct regression. Recurring amounts still require their captured-currency migration; a non-native budget cannot present that legacy fixed total as its own currency. Nonempty default changes remain unavailable until these consumers close |
| Recurring money (next) | Manual API/Web/Android creation, candidate confirmation, editing and lifecycle commands still require currency closure. The stored item must own baseline/last amounts; candidates, expense matching, anomalies, occurrence payments, budget/advisor/insight/dashboard totals, list/editor/Plan views and recycle/Owner projections are affected consumers. Retire current-default parsing/formatting and mixed-unit comparisons. Migration and audited adoption preserve money and versions; archive/restore and existing create/update receipts and Room intentions retain their original meaning. Direct producers include recurring command/query/candidate/occurrence/Web/recycle tests, migration/adoption tests and Android DTO/dispatcher/ViewModel/UI tests. No consumer is presumed unaffected before tracing its actual read |
