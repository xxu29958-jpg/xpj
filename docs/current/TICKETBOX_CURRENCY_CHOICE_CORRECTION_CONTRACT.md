...
| Budget aggregation and remaining consumers | Budget, income, recurring and goal projections reuse the existing FX owner; missing conversion stays unknown. Next: advisor spending inputs, shared monthly-report cards and their exact missing-rate recovery. Period reports, Web overview and Android Stats still sum some raw currencies; these remain an explicit following slice. Unused radar helpers are not evidence of a working insight entry. Nonempty default changes remain unavailable until these real consumers close |
