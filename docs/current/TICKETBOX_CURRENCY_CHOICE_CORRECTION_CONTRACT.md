...
| Next: budget money and save | API monthly PUT, Web form and Android `BudgetRepository` currently save without captured currency, OCC or a stable receipt. `BudgetCategory` has a composite FK to its monthly budget and can inherit that owner's currency; it needs no independent currency authority. Budget/period/advisor/recycle consumers, archive/restore and direct save producers must close together |
