...
| Remaining money consumers | Budget, advisor, monthly-report, income, recurring and goal candidates use the existing FX owner. Monthly/lifestyle Stats, Web overview/calendar/14-day trend and Android monthly offline projections still have raw cross-currency paths; these are the next consumer slice. Unused helpers do not prove a working insight entry. Nonempty default changes remain unavailable until these consumers close |
...
reports are in construction; the remaining Stats/overview consumers, nonempty
default-change command and historical correction journey remain open.
