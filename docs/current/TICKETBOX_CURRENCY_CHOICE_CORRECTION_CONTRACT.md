...
| Budget aggregation and remaining consumers | Confirmed expense/refund streams carry recorded currency; budget and income projections share the existing FX owner. Missing conversion stays unknown. Recurring totals now use captured currencies in the candidate implementation. Spending goals now share the budget projection reader. Period reports and advisor spending readers still require common-currency projection. Monthly-limit saves no longer evict advice because saved limits are not advisor inputs. Nonempty default changes remain unavailable until the affected consumers close |
