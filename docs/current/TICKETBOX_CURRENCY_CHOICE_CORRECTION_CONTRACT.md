...
| Recurring money (in construction) | Backend/Web candidate: required captured currency on create/edit/candidate confirmation; edits cannot relabel amounts. Existing idempotency rows retain accepted manual command receipts instead of replaying a later revision. Migration and audited adoption fill only unknown historical currencies, preserving money, versions and relationships. Candidates, anomalies, budget/advisor reservations and the Web overview reuse FX projection; missing inputs stay unavailable. Item/editor, linked payment and recycle views use their own recorded units. Review cannot rebase an incompatible raw draft; it retains the original and offers the current editor separately. Owner reminder operations consume dates/counts only. Cloud migration/API/Web validation and Android DTO/dispatcher/ViewModel/UI/reminder/Plan consumers remain open; false 404 settlement, unchecked receipts and obsolete Room intentions are not yet closed. Direct producers: recurring currency/receipt/projection and migration tests, recurring/Web/occurrence/recycle/adoption suites, plus Android contract/recovery/UI tests. No deployment or completion claim |
