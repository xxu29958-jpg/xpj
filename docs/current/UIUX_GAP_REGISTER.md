...
- Status: collapsed create tools true-device verified; populated/error states still pending.
...
- Resolution: the page now starts with a real catalog/alias overview derived
  ...
  merchant text; they affect directory visibility and future/stat grouping. The
  create forms now stay behind an on-demand tools section, with true-device
  evidence `294-settings-merchant-tools-collapsed.png` confirming the default
  page no longer exposes add forms.
