...
| Income plan revision → forecast → actual edit recovery | Server-declared months, immutable history and legacy scheduled aggregates retain distinct meanings. API/Web/Android and recycle use the intended month. Room preserves the original body, binding, key and OCC before the sole dispatcher; both recovery entrances retain refused commands and unsupported old intents cannot replay. Same-status/OCC exits must match the original month; known receipts precede OCC. Shared negotiation now carries actual version/binding evidence even when writing is blocked, leaving adoption and configuration decisions with the currency owner. | Active #377 at `ce047e2c`, including merged Capture main. Thirteen older formal threads are resolved; the remaining owner-action P2 has a production candidate, 31 short boundary cases and independent bounded review, but is not formally resolved. The new combined SHA awaits its own cloud qualification and protected integration. See `TICKETBOX_INCOME_PLAN_CONTRACT.md`; no income-accounting or full RC claim. |
