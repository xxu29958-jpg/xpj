...
| Income plan revision → forecast → actual edit recovery | Typed immutable revisions preserve monthly history; one query owner supplies whole-month estimates and scheduled-through-today values. API/Web/Android and recycle restore carry the declared month. The real Android editor saves original month, amount, binding, key and OCC to Room before dispatch; recovery remains readable when the management GET fails. Direct-first edit and Android active-head totals retire. | Candidate implementation, not yet runtime-qualified. Real Web received-income claim reproduced RED; five pure forecast/template cases pass. Existing frequency conversions and bounded one-time corrections retain unrelated history. PostgreSQL stable receipt/OCC/immutability and actual Android Room/UI cross-month tests are added. See `TICKETBOX_INCOME_PLAN_CONTRACT.md`; no income-accounting subsystem or confirmed-receipt claim. |
