...
**Current next action:** #371 closed the repository-weight interlude on independently qualified main `1d5cc293a94dd7e9b13fdf2a5442d505907d7d3b` (CI `34000818853`, CodeQL `34000818829`, actual Connected `34000818849`). Its pending construction notes are historical; the map is now available through the existing quality-guide/CI entry. Resume consumer delivery with the Android external-debt entry journey: keep Save within touch reach while the actual software keyboard is visible, reuse the current form and publication owners, and qualify the full input-to-local-acceptance interaction. The first test is ahead of production and requires observed cloud evidence. Physical process death, unsubmitted-text restoration and all other remaining packages stay open in this same Goal.
