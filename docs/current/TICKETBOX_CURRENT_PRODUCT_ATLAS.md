...
完整摸排日期：2026-09-19，源码基线为 `main 8e1aa228`；后续施工状态更新至 2026-09-20 的 `main 446a3c54`（#423，候选三门通过，合并后独立资格进行中）。日常安装仍对应 `0ac27032`，两者分开核实。原摸排的完整身份、来源和证据边界见[事实记录](../qualification/2026-09-19-product-assessment.md)，后续片的直接证据见各当前合同。
...
| CSV 与外币账单导入；Web，CSV batch owner | 保存批次、剩余行续办、历史日期汇率、逐单手动汇率、确认前复核保留。#422 已合并并独立资格化主线：原生购买/退款/退单/冲销识别、同账本重复匹配、缺原单或汇率证据复核及回原行续办；普通 CSV 与失败行导出继续可用。#423 已接通统一账务日和原始时间证据。 | 完整可携带数据和后续真实 RC 演练仍须交付；#423 的合并后独立资格进行中，单片资格不代替终点。 |
...
| 已确认事实、更正、退款、冲正；Facts/correction/offset | 原币/本位币、冻结 FX、历史更正、偏移事实、原单追溯、商品项/分摊和复合更正；退款/冲正导出的 FX 证据已在 main。#423 将根流水期间与查询时区分离，保留日期精度、历史与原始输入。 | 附件状态和完整出口不足；#423 合并后独立资格进行中。财务更正与回收站恢复继续分工。 |
...
1. **账务日期的共同事实已随 #423 合并，合并后独立资格进行中。** 原反例是同一已确认根流水随查询时区跨月。现在根流水与 offset、预算、目标、月报和导出采用共享账务日期；账本规则版本、日期精度、原始输入、旧草稿和回执保留。无时间证据的历史记录保持可查、可导、可沿原单更正，受影响期间明确不完整，不虚构原始日期。候选 `e5add049` 三门已通过，不能把它代替 `446a3c54` 的独立结果或最终 RC。
...
**当前施工先手：**保留并独立资格化已合并的 CSV 与账务日期成果，完成附件完整性、同账单续办和真实 Web/Android 文件连续性，再补包含关系、计划、资料和原件的可携带出口。已有读写、原始草稿、回执、历史和恢复能力不能因改造缩水；不能退回零散页面队列，也不能由分析 CSV 独自冒充完整数据出口。
