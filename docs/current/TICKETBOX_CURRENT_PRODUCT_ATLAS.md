...
| Planning/Insights: act on the unclassified records counted by Web data health | **FIX / implemented for qualification.** Separate existing pending/confirmed count links now lead to their records. Confirmed uses all months and the shared category caliber, excludes offset events, and preserves pagination, detail return and batch failure/success scope. Viewer remains read-only. The mixed count → new-rule action is retired; rule preview/application/rollback remains. Current slice: `TICKETBOX_INSIGHT_FACT_NAVIGATION_CONTRACT.md`; 7 pure navigation/query cases pass, PostgreSQL product postconditions await cloud execution. |
| Planning/Insights: inspect an exact major expense from its monthly report and return | **FIX / implemented for qualification.** The actual top-expense projection retains each identity in its fact link. Existing correction/return helpers allow only the reports origin and validated month. Two identical display rows with different IDs now navigate separately; 422/409/success retain the same origin through the existing form owner. Pure counterexamples moved from RED to PASS; real forms await PostgreSQL execution. |
...
| Backstage: local Owner first-use and uncertain pairing continuation | **Active #378 at `4c25d864`.** Real local code entry and original-code continuation replace installer-only guidance. Both generation entries share the initial/read/recovery gate; pending overrides an otherwise valid old-ledger view. The backend seed helper has a real bounded wait. Latest three P2s have correction candidates and remain formally open; earlier four are resolved. Prior `e8cfd762` passed all gates; current gates remain pending. Synthetic credential storage does not prove process-death WinCred durability or clean-Windows RC. |
...
The listed in-flight PRs remain candidate work. Their evidence is tied to their own heads; each final candidate and protected merge-main must qualify independently. #372 stays deferred while substantive capability gaps remain.
