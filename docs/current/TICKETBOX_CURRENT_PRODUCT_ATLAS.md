...
| FX scheduler → Owner operational status | A transient lease/session failure now leaves the worker available for its next scheduled tick. Owner status separates enabled configuration from actual thread liveness and startup configuration failure; errors are bounded messages and the raw configured URL is retired from this surface. | Implementation present, qualification pending. A source-bound loop probe on `a86d33ff` observed the escaping lease error; four narrow loop/liveness/configuration regressions passed after the fix without a database or network connection. Existing stored rates and manual refresh remain; the real Owner/PostgreSQL path still requires cloud qualification. |
