...
| Android external-debt create recovery | `PARTIAL`, #369 production and review closed; final candidate qualification open | `DebtCreationRepository` → bound Room Outbox → `CreateDebtDispatcher` → unchanged backend Debt owner | Original submitted payload/key/owner persist before network; local acceptance is not a Debt. Real Room close/reopen, original-key retry and shared pending/list-to-sheet consumers passed cloud Connected. The ordering FIX has observed RED/GREEN; `24ae61f0` passed all 2,081 unit tests, CodeQL and Connected but failed a test-fake complexity gate. Its test-only boundary split needs fresh qualification. Keyboard-visible interaction, physical process restart and unsubmitted editing-text restoration remain open claims |
...
| Relationships and submitted-intent recovery | Debt context is qualified; #369 final candidate `c8085a59` passed CI/CodeQL/107 actual Connected tests and is merged as `a841d2a8` with an identical tree. Independent merge-main gates remain pending. The next real-Room recovery tests are ahead of production: multiple failed creates are not yet human-identifiable in the shared screen | Submitted create keeps its original intent; a user can identify the intended failed record, retry it or explicitly discard its local intent. Existing splits/proposals/repayments/adjustment/forgiveness/void and their canonical totals survive the integrated journey |
...
**Current next action:** collect #369's independent merge-main qualification while the next slice first proves the missing identifiable-recovery behavior through its existing real screen, ViewModel, create producer and Room. Production repair follows the observed counterexample and retains the original retry/drop authority. The keyboard-visible form check stays in the consumer-journey package; a non-keyboard screenshot cannot close it. The other packages are remaining delivery obligations, not permission to start parallel rewrites.
