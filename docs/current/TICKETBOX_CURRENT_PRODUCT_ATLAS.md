...
3. **数据健康尚不等于原件完整性。** 主线原图主要查路径/存在，健康主要识别缺路径/清理标记；digest 和备份 hash adapter 已有。已用临时文件复现摘要不符仍下载/识别，以及清理提交失败前文件已被删除。[原件完整性与同账单续办](TICKETBOX_ORIGINAL_INTEGRITY_CONTRACT.md)候选已接通校验读取、只读状态、同账单补回、旧件人工核验、持久清理及续办命令，保留实际原件身份与财务历史。窄文件/命令验证通过；数据库原子性、真实 Web/Android 入口和文件/离线续办仍待完整接通与资格化，不能算主线完成。正常手工无图记录保持有效，历史无摘要原件继续可读且明确未核验。
