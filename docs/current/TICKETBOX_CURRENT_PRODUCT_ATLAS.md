...
| Android external-debt create recovery | `STRONG_SLICE`, #369 candidate and independent main qualified; identifiable failed recovery continues in #370 | `DebtCreationRepository` → bound Room Outbox → `CreateDebtDispatcher` → unchanged backend Debt owner | Original submitted payload/key/owner persist before network; local acceptance is not a Debt. Candidate `c8085a59` and identical-tree merge `a841d2a8` independently passed CI, CodeQL and actual Connected. Real Room close/reopen, original-key retry, ordering RED/GREEN and shared pending/list-to-sheet consumers are qualified. Keyboard-visible interaction, physical process restart and unsubmitted editing-text restoration remain open claims |
...
| Android offline mutation publication | `STRONG_SLICE`; #370 external-debt recovery context awaits qualification | Room Outbox → registered dispatchers → backend owners | Preserve full mutation-type/label/dispatcher coverage. #370 observed two real-Room/consumer RED cases for missing record identity. The existing typed Debt projection now supplies original counterparty/amount/direction/note to both recovery entries and the shared explicit-discard preview; retry/drop owners are unchanged. Current-head cloud GREEN and visual/review acceptance are still required. Other same-type mutation families remain in the broader recovery journey; never expose raw keys or silently drop intents |
...
| Relationships and submitted-intent recovery | Debt context and #369 submitted-intent continuity are qualified on both candidate `c8085a59` and identical-tree main `a841d2a8`. #370's two real-Room/consumer RED cases are observed; original-record recovery presentation is implemented but not yet cloud/visually qualified | Submitted create keeps its original intent; a user can identify the intended failed record, retry it or explicitly discard its local intent. Existing splits/proposals/repayments/adjustment/forgiveness/void and their canonical totals survive the integrated journey |
...
**Current next action:** #369's independent merge-main gates are closed. #370 now needs exact-current-head GREEN, actual recovery/confirmation frame inspection and bounded review after its observed real consumer RED. It retains the original retry/drop authority. The keyboard-visible form check stays in the consumer-journey package; a non-keyboard screenshot cannot close it. The other packages are remaining delivery obligations, not permission to start parallel rewrites.
