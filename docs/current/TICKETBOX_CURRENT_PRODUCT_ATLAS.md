...
完整摸排日期：2026-09-19，源码基线为 `main 8e1aa228`；后续施工状态更新至 2026-09-20 的 `main 50a128d1`（#424，候选与合并后主线各自三门通过）。日常安装仍对应 `0ac27032`，两者分开核实。原摸排的完整身份、来源和证据边界见[事实记录](../qualification/2026-09-19-product-assessment.md)，后续片的直接证据见各当前合同。
...
| CSV 与外币账单导入；Web，CSV batch owner | 保存批次、剩余行续办、历史日期汇率、逐单手动汇率、确认前复核保留。#422 已合并并独立资格化主线：原生购买/退款/退单/冲销识别、同账本重复匹配、缺原单或汇率证据复核及回原行续办；普通 CSV 与失败行导出继续可用。#423 已接通统一账务日和原始时间证据。 | 完整可携带数据和后续真实 RC 演练仍须交付；#423 的合并后独立资格已通过，单片资格不代替终点。 |
...
| 已确认事实、更正、退款、冲正；Facts/correction/offset | 原币/本位币、冻结 FX、历史更正、偏移事实、原单追溯、商品项/分摊和复合更正；退款/冲正导出的 FX 证据已在 main。#423 将根流水期间与查询时区分离，保留日期精度、历史与原始输入。 | 附件状态和完整出口不足；#423 合并后独立资格已通过。财务更正与回收站恢复继续分工。 |
| 币种选择与换算恢复；Money/binding/FX | 显式初始币种、可改默认、记录金额不改标签、多币种精度、历史/单据汇率续办、外币固定支出创建。 | 补全期间/计划/导出消费者的一致性；保留缺汇率时的输入和原任务，不能统一 CNY 简化。 |
| 欠款、应收、拆账、还款；Relationships，Android/Web | 外部往来、成员拆账接受/结算、幂等还款、调整/免除/作废事实和 fold；原提交重试、债务账单图片识别的原家庭身份绑定、还债目标关联；识别图片为瞬态建议输入。完整八类时间线、分页与关联还款定位已接入 Web/Android 候选，保留单笔撤销、原币和实际确认额。 | 时间线候选尚待 exact 云端及合入资格；已接受拆账发生部分退款后，双方重议与结算仍需交付，不能由补链接代替。 |
...
| 原件、数据出口、宿主；Assets/Portability/Windows | 鉴权原图、上传 digest、受限 resolver、缩略图/清理、备份原件 hash 与 inventory；#424 已交付完整性状态、同账单补回、旧件核验和持久清理续办，主线独立资格通过。#425 已有含历史/关系/原件的完整出口候选。Fresh-G2 关闭，Desktop 诊断已有实现。 | 出口候选仍须当前 HEAD 合入资格；全局孤立原件处置及当前阶段可执行数据安全边界继续按实际风险闭合。完整 restore/upgrade 等继续 HOLD。 |
...
1. **账务日期的共同事实已随 #423 合并，合并后独立资格已通过。** 原反例是同一已确认根流水随查询时区跨月。现在根流水与 offset、预算、目标、月报和导出采用共享账务日期；账本规则版本、日期精度、原始输入、旧草稿和回执保留。无时间证据的历史记录保持可查、可导、可沿原单更正，受影响期间明确不完整，不虚构原始日期。候选 `e5add049` 与主线 `446a3c54` 各自通过 CI、CodeQL 和 Connected；这些结果仍不代替最终 RC。
...
3. **原件完整性与同账单续办已随 #424 合并。** 原先摘要不符仍下载/识别、清理提交失败前文件已删除的反例已修复。[原件完整性与同账单续办](TICKETBOX_ORIGINAL_INTEGRITY_CONTRACT.md)接通校验读取、只读状态、同账单补回、旧件人工核验、持久清理及 Web/Android 续办，保留实际原件身份与财务历史。最终候选 `019711bc` 的 CI、PostgreSQL、CodeQL 和 Connected 均通过，受保护合并 `50a128d1` 与其源码树一致。Web 原文件重载、断网及 ACK 丢失续办已用真实 Chromium 验证；Android 离线准入和原键续办保留，实际发送校验服务端能力。正常手工无图记录保持有效，历史无摘要原件继续可读且明确未核验。单片资格不代替最终 RC。
4. **数据主权的完整出口正在施工。** 已合并的 CSV 未覆盖完整修订、关系、计划、资料库和原件；备份代码不等于可恢复产品。[可携带数据出口](TICKETBOX_PORTABLE_DATA_OUTLET_CONTRACT.md)候选已接通 API/Web 下载、全部授权状态与历史、账号收件箱/往来、原件及历史回执引用。结构数据使用独立只读快照，余额复用原领域计算；文件缺失不抹掉财务记录，包内分别说明原件状态。143 项短测试已通过（含 107 项既有 Web 路由检查）；8 项真实 PostgreSQL 快照、并发权限和下载用例仅收集，尚未据此宣布可交付。现有 CSV、原件操作与各端未提交意图保留。Internal Beta 允许的诊断/导出/冷备边界仍需实际闭合，完整 Windows 生命周期不因此重开。

### 完整业务任务

5. **往来不能只见余额和还款列表。** [往来持续使用](TICKETBOX_RELATIONSHIP_CONTINUITY_CONTRACT.md)候选已用现有事实补齐八类时间线；Web 不再止于前 20 笔，Android 保留原页重试、绑定隔离和已接受命令结果；部分确认展示实际金额，跨页定位在同一次查询内完成。已接受拆账/来源退款提示可沿权限校验进入往来。双方重议仍继续施工：接收方流水本身已有独立 offset 能力，必须核对已有退款，避免再减一次；已还部分不能伪造为未发生或借作废消除。保留原约定、消费和退款，不自动把退款当还款、绕过同意或另造余额 owner。
...
当前施工：#422 原生 CSV 续办与 #423 统一账务日期的主线独立资格已通过，#424 原件完整性已受保护合并为 `50a128d1`。按用户最新排序，依次完成现有 #425 可携带出口、#426 往来时间线、#427 拆账约定的集成、当前失败修复、所需门禁和合并；主线验证沿既有 CI 执行，在后续合入节点核对，不为等待停止其余已授权集成。完整五域、Backstage、保留条件和下列终点继续有效，不构成这几张 PR 的额外合并前置条件。
...
**当前施工先手：**优先收口现有 #425～#427，不新增叠层、不扩大重审；保留后续已完成代码，待现有候选合并后按完整目标继续。已有读写、原始草稿、回执、历史和恢复能力不能因改造缩水；不能退回零散页面队列，也不能由分析 CSV 独自冒充完整数据出口。
