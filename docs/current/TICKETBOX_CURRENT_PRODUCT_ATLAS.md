...
**Durable upload continuity, active #385.** The earlier memory-only launch and multipart sender are now retired in the candidate. Accepted original files/order/identity persist through the existing bound Room Outbox before Activity handoff acknowledgement; the sole dispatcher and backend atomic idempotency receipt preserve retry and capacity-tail semantics. Parent-integrated source `d2251190` passed CI/CodeQL/Connected after correcting a stale resource annotation. Formal P2 `PRRT_kwDOS5LrfM6f1H8O` remains active: a committed upload whose immediate read-back is unavailable must recover the unsubmitted original enrichment task on later receipt replay. Test-first `4f4bb1da` reproduced the missing execution in real PostgreSQL. Candidate `f9ec6d84` adds original durable task input and replay; all three real-db shards pass, while the new shared-failure dependency creates a rejected cycle. Corrected source `ae2fc09e` removes that worker coupling and is in fresh qualification; both formal P2 threads remain unresolved pending final proof. [The slice contract](TICKETBOX_UPLOAD_INTENT_CONTINUITY_CONTRACT.md) retains all before/after impact, failures and retirement evidence. Actual OS process interruption and the full capture task remain separate acceptance claims.
...
**Next bounded capability counterexample: debt-bill image preparation across a household switch.** Both real launchers suspend preparation before a request that currently binds the then-active household. The current HTTP response guard does not cover that earlier interval. Cloud source `4813e4c6` executed 2,244 JVM cases with exactly this new counterexample failing; XML confirms the original bytes reached a request bound to the new household. Candidate `796dc10e` carries one original attempt through preparation, bound request and result; its actual Android-fast job `101672987844` passed compilation, JVM, Lint and Detekt. A repository-weight failure in the direct test producer is corrected by retiring its one-use fake factory, retaining all 35 cases and 108 assertion lines. The Capture dependency is integrated at `ae2fc09e`; final combined-source qualification and both protected merges remain pending. [The debt-bill binding contract](TICKETBOX_DEBT_BILL_BINDING_CONTRACT.md) maps both entrances, the unique recognition/creation owners, old result exits and affected producers. This remains in Relationships/cross-client continuity; it does not change submitted-intent semantics or open a draft-storage/Windows lifecycle project.
