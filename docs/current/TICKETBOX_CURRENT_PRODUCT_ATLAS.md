...
3. **数据健康尚不等于原件完整性。** 主线原图主要查路径/存在，健康主要识别缺路径/清理标记；digest 和备份 hash adapter 已有。已用临时文件复现摘要不符仍下载/识别，以及清理提交失败前文件已被删除。[原件完整性与同账单续办](TICKETBOX_ORIGINAL_INTEGRITY_CONTRACT.md)已形成隔离读取候选，下载/OCR/缩略图等 68 项窄测通过；状态展示、原件补回、历史核验、可靠清理和多端续办仍未闭合，不能算主线完成。复用同一附件关系和提交 owner。正常手工无图记录保持有效，历史无摘要原件继续可读且明确未核验。
...
当前施工：[原生 CSV 财务事件续办](TICKETBOX_CSV_EVENT_CONTINUATION_CONTRACT.md)已通过候选资格并以 #422 合并，`44d684fb` 的主线 CI、CodeQL、Connected 独立通过。[统一账务日期与原始时间意图](TICKETBOX_ACCOUNTING_CALENDAR_CONTRACT.md)的时间内核、存储采用、命令/回执、共享期间查询、日历治理、Web/Android 表单、原始草稿与缺省月份，以及 CSV、拆账、OCR 和退款入口已集成在隔离候选。没有时间依据的历史单据仍可查找、导出和沿原单更正；受影响期间明确不完整，其他分类继续可用。账本规则变更不改旧草稿、回执、明确选择的月份或仅修改商家的原时间来源。原反例与最终定向集成验证已通过，首轮诊断云端暴露的合同/夹具/旧 schema 接线问题已修复，正在进入新 exact head 资格化；数据库及完整多端资格尚未成立，不能提前算作主线能力。其余目标、保留条件和后续顺序继续有效。
