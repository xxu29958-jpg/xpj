...
| AI advisor | `PARTIAL`, shared readiness implemented and awaiting cloud qualification | Existing provider factory + consent/role readiness → Owner, Web, API and Android error recovery | Configuration validity and actual call history stay separate; secrets remain lifecycle-owned |
...
| Income plan revision → forecast → actual edit recovery | Server-declared months, immutable revision forecasts, legacy scheduled aggregates, read/write protocol negotiation and original offline edit recovery share the real consumers. Same-status archive/restore must match the current revision and original intent month, including OCC races; edit receipt HIT remains prior to OCC. | Active #377 at `fed1a02f`. Prior `ef54e1a5` passed CI `34029468563`, CodeQL `34029468564`, Connected `34029468584` (114 tests). Nine older formals are resolved; the final same-status/month P1 has a correction candidate and remains formally open. Its 16 pure cases pass; current exact cloud gates and protected integration remain pending. No income-accounting or RC completion claim. |
