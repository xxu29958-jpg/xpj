...
| 欠款、应收、拆账、还款；Relationships，Android/Web | 外部往来、成员拆账接受/结算、幂等还款、调整/免除/作废事实和 fold；原提交重试、债务账单图片识别的原家庭身份绑定、还债目标关联；识别图片为瞬态建议输入。#426 补齐八类时间线、分页与关联还款定位。后继候选接入双方新约定、反复重议、已付后的返还、跨账本应付发现和三类约定历史，保留原付款、免除、原币和实际确认额。 | 两个候选均须取得各自 exact 云端及合入资格；Android 新约定未入队表单目前只在任务 ViewModel 保留。跨账本返还加入另一账本的还债目标、经济事件归属与个人账务核对便利性仍需加强。 |
...
当前施工：[原生 CSV 财务事件续办](TICKETBOX_CSV_EVENT_CONTINUATION_CONTRACT.md)的 #422 主线独立资格已通过。[统一账务日期与原始时间意图](TICKETBOX_ACCOUNTING_CALENDAR_CONTRACT.md)的完整候选 `e5add049` 已通过 CI、CodeQL、Connected 及有界复核，#423 受保护 squash 为 `446a3c54`，两者源码 tree 相同；该 main 的独立三门均已通过。附件 #424、可携带出口 #425、往来时间线 #426 与后继新约定候选依次施工，未取得各自 exact 资格前不冒称已合入。新约定本地 138 项后端/Web/portable 短测、最后 Web 23 项和 Android 97 项定向验证通过，数据库迁移/并发及 Connected 留给现有云端门；不反复等待或刷新 CI。完整五域、Backstage、保留条件和下列终点继续有效。
...
**当前施工先手：**保留已合并并独立资格化的 CSV 与账务日期成果，按实际失败和合入节点处理附件、完整出口、时间线与新约定候选资格；继续经济关系与个人核对的剩余消费者，再补计划历史和持久储蓄安排。已有读写、原始草稿、回执、历史和恢复能力不能因改造缩水；不能退回零散页面队列，也不能由分析 CSV 独自冒充完整数据出口。
