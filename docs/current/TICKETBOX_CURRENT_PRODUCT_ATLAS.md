...
**Current next action:** the Owner's September 6 priority remains whole-system capability completion before small interaction details. Monthly recurring fulfillment and FX task recovery passed candidate and independent merged-main qualification in #373 and #374. The recurring [working contract](TICKETBOX_RECURRING_OCCURRENCE_CONTRACT.md) retains its boundaries. Owner business recovery consolidation is implemented on `codex/recycle-bin-consolidation-20260906` for cloud qualification and integration. Income-plan history and advisor readiness remain below in the same Goal. Draft #372 remains unmerged on its separate branch, with its local test follow-up preserved for later qualification. No detail-level image, keyboard or cosmetic task gates the capability work, and the original full Goal remains active.
...
| Owner recycle surface → canonical restore | Owner navigation now enters the existing Web recycle bin with actual business identity and ledger permissions, including merchant catalog. The separate Owner route, projection, restore dispatcher and sole money helper are retired. Ledger governance restore stays on `/owner/ledgers`. | Implemented in the isolated consolidation slice; cloud qualification pending. The rendered-navigation RED showed the old Owner URL; that same probe now passes. One bounded review of `86cdf046` found no P1/P2; all 2,998 snapshot files retained their hashes. The real installed-Owner → Web identity → stale/valid merchant restore journey is added; existing canonical role, retention, money and ledger-unarchive regressions remain. No new entity restore writer. |
| Advisor owner → Owner/Web/API readiness | Read-only readiness reuses the provider factory's model/URL/credential validation and the original consent/role rules. Owner status distinguishes valid configuration from the observed last call; Web requests require the same readiness, and API/Android expose an actionable terminal configuration error before financial input construction, quota reservation or outbound execution. Raw configured URLs are retired from status. | Implemented in the isolated readiness slice; cloud qualification pending. A real Owner-template RED reproduced the callable claim with a missing model. Six invalid-configuration cases, consent/role checks and pre-input refusal pass without database/network access. API/Web correction and Android terminal-state regressions are added for cloud execution. No new configuration editor, external call, quota policy or AI budget writer. |
