...
| Manual expense entry on Web | `PARTIAL`: native consumer implemented; cloud qualification in progress | `create_manual_expense` → native Web form, API and Android manual-entry sheet/Outbox | Native entry, member/viewer admission, currency parsing, device-scoped replay and pending-FX continuation exist. Cloud exposed stale-ledger, revoked-role and re-enrolled-browser submission; fixes retain the original form binding and revalidate live command authority. Final cloud qualification and draft continuity remain required |
| External-debt context: remember why this obligation exists | `PARTIAL`, current construction | Debt create/query owners → Web and Android entry/detail | Persist the optional context in the Debt owner, carry it through the shared create fingerprint and canonical responses, show it in both clients, and retain it after validation/transport failure. Retire the Web input-without-writer behavior. Exact qualification remains pending |
...
### Manual expense entry — qualification in progress
...
Desktop first-use explanation and expired-code recovery links remain companion gaps after this identity slice. Web manual entry is implemented and under qualification as described above. Installation currency adoption remains a Desktop-only ceremony. These facts do not authorize reopening installer, account recovery, upgrades or other Windows HOLD work.
