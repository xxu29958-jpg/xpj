...
| Relationships and submitted-intent recovery | Debt context is qualified; #369 production and its bounded review are closed pending the test-only final gate. Multiple same-type failures are not yet human-identifiable in shared recovery | Submitted create keeps its original intent; a user can identify the intended failed record, retry it or explicitly discard its local intent. Existing splits/proposals/repayments/adjustment/forgiveness/void and their canonical totals survive the integrated journey |
...
**Current next action:** finish #369's exact candidate and merge-main qualification, then close identifiable shared recovery through its actual existing screen and callbacks. The keyboard-visible form check stays in the consumer-journey package; a non-keyboard screenshot cannot close it. The other packages are remaining delivery obligations, not permission to start parallel rewrites.
