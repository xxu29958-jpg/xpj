...
| Relationships: original debt-adjustment submission and recovery | **FIX / active #379.** Persist original signed amount/reason/OCC/key/binding before dispatch; expose pending recovery in detail and both sync entrances; retire the direct adjustment writer. Actual test-first cloud reproduced failed original-command recovery. Current `379a8ce1` corrects a first-claim test expectation and a Connected test interface wiring error observed on `33c2888e`; its exact gates remain pending. Other direct debt commands remain separate probes without demonstrated duplicate posting. |
