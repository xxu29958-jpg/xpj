...
**Current next action:** the Owner's September 6 priority remains whole-system capability completion before small interaction details. Monthly recurring fulfillment has passed candidate qualification and merged in #373, governed by the [working contract](TICKETBOX_RECURRING_OCCURRENCE_CONTRACT.md); independent main qualification is running. The next implemented capability is FX task recovery and truthful Owner runtime status, isolated on `codex/fx-scheduler-recovery-20260906` for cloud qualification and integration. Income-plan history, recovery consolidation and advisor readiness remain below in the same Goal. Draft #372 remains unmerged on its separate branch, with its local test follow-up preserved for later qualification. No detail-level image, keyboard or cosmetic task gates the capability work, and the original full Goal remains active.
