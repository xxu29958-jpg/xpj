...
| Relationships: original debt-adjustment submission and recovery | **Active #379, root fixes in progress.** Original signed amount/reason/key/OCC/binding remain durable before the sole dispatcher. Test-first `206c023a` actually completed 2,130 JVM cases with six intended failures covering Retry scheduling, unsupported/overlong payloads and stale-write exclusion. Actual Connected completed 120 cases with four failures: original Save/Retry scheduling and three retained balance consumers after real Room Done. The latter passed initial balance, submission, detail-VM close, drain and persisted-Done prerequisites before refresh timed out. Closure includes DebtList, Receivables, DebtGoal, CreateDebtGoal candidates and RepaymentDraftInbox candidates; currency-only readers are separately excluded by unchanged currency semantics. Formal threads and final integrated qualification remain open. |
