...
| Facts: recover a submitted composite correction across interruption | **Active #382.** Required Room/Outbox ownership now persists original payload/key/OCC/binding before Save dispatch; detail and both Sync entrances preserve original Retry and canonical refresh. Exact `4bc14c98` passed CI/CodeQL; actual Connected executed 122 tests with two failures and zero skips: the shared overview falsely reported unproven legacy DONE as settled, and the detail test selected a non-editable reason label before Save. Own candidate `b7150a7c` implements the bounded overview correction and real editable targeting while preserving the original Save/Retry/Room assertions. Integrated parent `f614ceef` includes the current Income/Web/Debt status; new candidate and protected-main qualification remain pending. See the existing Facts contract. |
| Planning/Insights: act on the unclassified records counted by Web data health | **Active #380.** Pending/confirmed count links use canonical ledger/category scope; confirmed spans all months, preserves pagination and retains viewer restrictions. The mixed count → new-rule exit is retired. Current `3eb75aed` includes the reviewed parent semantics, report quality correction and merged Income main. The report P2 threads were resolved/read back; this new head and protected main still require their own qualification. |
| Planning/Insights: inspect an exact major expense from its monthly report and return | **Active #380.** Exact fact identity and original ledger/month survive 422/409/success. Real return feedback is consumed, while month/ledger/identity navigation strips stale one-time feedback. The combined 23 short cases passed; intermediate `31305eca` passed weight and Desktop but its Windows subprocess gate timed out. The report P2 threads were resolved/read back. Current `3eb75aed` integrates merged Income main. The latest controller snapshot reports its Windows served-Web layout case at 1180×760 failing after two fresh-CDP-session socket timeouts (10 passed, one failed), with bounded diagnosis underway and no new product FIX established. Other lanes were still running at that snapshot; this head and protected integration remain unqualified. |
| Relationships: original debt-adjustment submission and recovery | **Active #379, formal review underway.** Original signed amount/reason/key/OCC/binding persist before networking; detail and both Sync entrances share the recovery owner. Prior own candidate `e6a36224` passed all gates. Current `75888c5e` normally merges Web `3eb75aed` with the reviewed Income/Web consumers; its exact qualification and protected-main qualification remain. Old integrated `663ab431` failures retain their actual dispositions in the PR, including the now-corrected report quality regression. |
...
| Capture: return to a saved CSV batch and finish its actual remainder | **Active #384.** Native preview/apply stores the original batch, but the hub cannot find it after leaving; lease refusal also loses the detail. Exact test-first `9f7aaf2c` produced six real PostgreSQL task failures plus three pure behavior failures. Implementation now reuses existing batch/query/lease/row-idempotency owners, exposes bounded history pagination and actions from actual valid/applying rows, and preserves error receipts without futile Retry. Production and integrated qualification remain pending. |
...
Only the rows explicitly marked active remain candidate work; closed rows retain their bounded qualification. Their evidence is tied to their own heads; each final candidate and protected merge-main must qualify independently. #372 stays deferred while substantive capability gaps remain.
