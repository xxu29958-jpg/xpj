...
| Android external-debt create recovery | `PARTIAL`, #369 candidate implemented; qualification open | `DebtCreationRepository` → bound Room Outbox → `CreateDebtDispatcher` → unchanged backend Debt owner | Original submitted payload/key/owner persist before network; local acceptance is not a Debt. Shared pending and existing sync recovery consumers are wired. Real Room/retry/consumer cloud results and the admitted access/queue ordering fix remain open; physical process restart and unsubmitted editing-text restoration are not claimed |
