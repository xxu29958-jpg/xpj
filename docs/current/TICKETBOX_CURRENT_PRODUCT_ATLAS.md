...
| FX scheduler → Owner operational status | A transient lease/session failure now leaves the worker alive to attempt later scheduled ticks. An unexpired lease may skip a tick; successful work resumes when execution is admitted. Owner status separates enabled configuration from actual thread liveness and startup configuration failure; errors are bounded messages and the raw configured URL is retired from this surface. | Implementation present, qualification pending. A source-bound loop probe on `a86d33ff` observed the escaping lease error. Bounded review admitted the next-tick guarantee as too strong: the existing lease remains authoritative, the UI promise is corrected, and the targeted regression covers a held lease before later recovery. Existing stored rates and manual refresh remain; the real Owner/PostgreSQL path still requires cloud qualification. |
| Owner recycle surface → canonical restore | Owner navigation now enters the existing Web recycle bin with actual business identity and ledger permissions, including merchant catalog. The separate Owner route, projection, restore dispatcher and sole money helper are retired. Ledger governance restore stays on `/owner/ledgers`. | Implemented in the isolated consolidation slice; cloud qualification pending. The rendered-navigation RED showed the old Owner URL; that same probe now passes. The real installed-Owner → Web identity → stale/valid merchant restore journey is added; existing canonical role, retention, money and ledger-unarchive regressions remain. No new entity restore writer. |
