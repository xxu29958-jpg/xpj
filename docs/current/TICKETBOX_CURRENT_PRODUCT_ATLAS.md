...
| Public admin API exposure | `RETIRE` / admitted FIX, implementation candidate | Shared local governance boundary → Admin, maintenance and admin pairing; local Owner and remote ledger-scoped product consumers remain | Public opt-in retirement is in construction under [the local governance contract](TICKETBOX_LOCAL_GOVERNANCE_BOUNDARY_CONTRACT.md); exact candidate cloud qualification remains pending. No remote admin UI or Windows lifecycle expansion |
...
| Backstage consolidation and capability readiness | The current map still carries duplicate recycle/admin/developer surfaces and partial advisor/scheduler readiness. Public admin opt-in retirement is an admitted FIX and implementation candidate under the [local governance contract](TICKETBOX_LOCAL_GOVERNANCE_BOUNDARY_CONTRACT.md); cloud qualification remains pending | Verify current consumers, retire the duplicated product surfaces/writers, and expose useful readiness/health plus authorized recovery. Do not invent controls merely because an internal API exists or move secrets/host authority into Web |
