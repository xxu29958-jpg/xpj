...
**Durable upload continuity, active #385.** Accepted original files/order/identity persist through the existing bound Room Outbox before Activity handoff acknowledgement; the sole dispatcher and backend atomic receipt preserve retry and capacity-tail semantics. The original task-input/replay correction passed all three cloud gates at ae2fc09e, and both associated formal threads were resolved and read back. Pre-merge review then admitted two further P2s: missing local originals were misreported as saved, and permanent key refusal still admitted Retry. Test-first af93cf2d reproduced three JVM failures and the exact atomic SQL bypass. Source 17824a83 retires the separate saved-message writer, discloses missing originals immediately, and blocks permanent retries in the shared predicate and atomic storage update. Fresh final qualification and formal resolution remain pending. [The slice contract](TICKETBOX_UPLOAD_INTENT_CONTINUITY_CONTRACT.md) retains all before/after impact, failures and retirement evidence. Actual OS process interruption and the full capture task remain separate acceptance claims.
