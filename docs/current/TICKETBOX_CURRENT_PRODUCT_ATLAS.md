...
|---|---|---|
| First use and household entry | Invitation handoff, real local Web identity and ended-browser history are integrated. Desktop local binding entry and original-code continuation are implemented with short Controller/Edge evidence; candidate and merged-main qualification remain | Owner/member/viewer can enter the intended household, understand where data lives and recover from an expired/failed connection without losing entered intent or gaining another identity |
| Capture, confirmed facts and reference work | Upload/review, manual entry, FX recovery, correction/offset and reference owners exist. Web manual draft continuation is qualified; integrated ordinary-user journeys remain | Actual supported capture/import → review → confirmation → search/detail/reference/correction paths work with evidence, role restrictions and recoverable errors; no duplicate command owner |
...
| Backstage consolidation and capability readiness | Recycle consolidation, Advisor readiness and FX worker recovery are qualified; remaining dormant admin/developer surfaces need a concrete consumer decision | Verify current consumers, retire the duplicated product surfaces/writers, and expose useful readiness/health plus authorized recovery. Do not invent controls merely because an internal API exists or move secrets/host authority into Web |
...
| --- | --- | --- |
| Desktop first use → local device authorization → unknown-result continuation | Healthy local installations expose the existing Owner code task without requiring a phone URL. The entry explains local data and Owner/member/invitation meaning. Controller projects pending original-code recovery, including rebind after the old ledger disappears; UI retains input and respects existing proof expiry. The Windows cloud job reuses the owned native PostgreSQL test environment for actual pairing/BFF/served-Web evidence and rejects skipped cases. | Candidate implementation. Entry and continuation reproduced RED, including the admitted rebind P2. Controller/UI/browser regression passes 94 short cases; two live-backend cases skipped locally and are explicitly routed to the new cloud gate. Trigger regression turned green; GitHub workflow contract audit passed. Actual 820-pixel frame inspected and narrow layout exercised. See `TICKETBOX_DESKTOP_FIRST_USE_CONTRACT.md`. Cloud/live-backend qualification is pending; no process-death, lifecycle or final RC claim. |
...
| Income plan revision → forecast → actual edit recovery | Candidate `ef54e1a5` passed its own CI `34029468563`, CodeQL `34029468564` and Connected `34029468584` (actual 114 tests). Focused test-first cases proved protocol-refusal retention, fixed-date forecast meaning and read epoch skew before correction. The seven original and two read-skew formal threads are resolved with readback. One new formal P1 is admitted: a same-status archive/restore for a different declared month must not return success; first-read and OCC-race paths are under correction. Parent integration and this final fix still await their exact gates. See `TICKETBOX_INCOME_PLAN_CONTRACT.md`; no income-accounting subsystem or full RC claim. | Candidate `4fcaaeab` passed CI/CodeQL and the real Income Room/UI case; Connected still failed at the unchanged Debt Save case. Candidate f942609d passed CodeQL and JVM tests but failed Android static checks and three Connected Debt cases. Test-first f4580339 reproduced all three protocol-refusal cases as expected failed, actual done. Candidate e7ba9166 passed CI and CodeQL; its actual Connected protocol/recovery cases passed, but Save still failed. All seven formal threads are now resolved with readback. Diagnostic e51ce797 proved that a later viewport change invalidated the pre-capture Save visibility assertion; the test now locates Save after capture and keeps one real touch plus submission/freeze/local-acceptance checks. Candidate ce192c10 passed CI, CodeQL and all 114 Connected cases. Two new formal read-skew findings remain open: the legacy total must retain its scheduled meaning and new Android must check the read epoch before requiring new forecast fields. A fixed-date route case and two cloud JVM cases reproduced these failures; the explicit expected field, actual consumer mapping and bounded read negotiation are corrected and await their own qualification. Real-router protocol regression passes 19 short database-free cases. See `TICKETBOX_INCOME_PLAN_CONTRACT.md`; no income-accounting subsystem or full RC claim. |
...
| --- | --- |
| Capture: resume a shared image batch after capacity refusal | **FIX / PARTIAL.** `PendingRoute` consumes A/B/C; the existing upload loop stops at B and its retry retains B only. Keep the remaining batch with the existing upload task, preserve origin ledger and permissions, resume B/C with visible unfinished work and explicit abandonment. Retire the whole-batch handoff that only retains one image. Falsifier: A/B/B/C after one capacity interruption; A is not sent twice. Original images and accepted attachments are not proved lost. |
| Facts: recover a submitted composite correction across interruption | **FIX / PARTIAL.** The real `ExpenseCorrectionRepository` first sends before its IOException fallback persists the original command. Publish the submitted scalar/items/splits/reason/key/OCC/binding in existing Room before networking; use the existing `CorrectExpenseDispatcher` as sole sender and retain fact/history/insight refresh consumers. Retire this command's first-send bypass. Falsifier: inspect original row while first request is suspended, then reopen and recover the same command, including committed-response loss. No duplicate correction or confirmed-fact loss has been demonstrated. |
| Planning/Insights: act on the unclassified records counted by Web data health | **FIX / PARTIAL.** Consume the existing pending/confirmed components separately. Pending already has a matching filter; confirmed needs the same all-month Expense scope, missing-category predicate, pagination, detail return and any existing batch error/return fields. Viewer remains read-only. Replace the mixed count → new rule primary action; retain rule preview/application/rollback. Falsifier includes previous-month confirmed, `none`, and valid `其他`, then direct correction and reduced health count without creating a rule. |
| Planning/Insights: inspect an exact major expense from its monthly report and return | **FIX / PARTIAL.** Preserve the Expense identity already returned by `top_expenses_for_month`; link the existing fact/correction chain and admit bounded reports/month return context. Retire identity-free report row projection. Two visually identical records must open their respective facts; success, 422 and 409 must preserve the original report month and ledger. Existing fact writer and permissions remain authoritative. |
| Relationships: original debt-adjustment submission and recovery | **FIX / active #379.** Candidate `1f3bd2c5` reproduced response-loss recovery failure in CI `34029178586`, Android job `101475482566`: 2,097 JVM tests, one failed original-adjustment recovery case, no errors or skips. The client minted a new key while the server required the original command. Implementation uses existing Room/Outbox, original OCC/key/body/binding, detail and both global sync entrances, and retires the direct adjustment writer. See `TICKETBOX_DEBT_ADJUSTMENT_CONTINUITY_CONTRACT.md`; production and Room/UI completion are not yet qualified. Other direct debt commands remain a separate continuity probe, with no duplicate posting demonstrated. |
| Backstage: local Owner first-use and uncertain pairing continuation | **Active #378.** Real local code entry and original-code recovery replace installer-only guidance. Current candidate `e8cfd762` includes bounded pairing producer scope and test-database identity selection fixes; it awaits its own final gates and integration. Prior synthetic in-memory identity tests do not prove process-death WinCred durability or clean-Windows RC. |
...
Income candidate `ef54e1a5` has now passed its own CI `34029468563`, CodeQL `34029468564` and Connected `34029468584` after correcting legacy scheduled-read meaning and the Android read epoch gate. The two formal read findings are resolved with readback; the new same-status/month P1 and protected integration remain open. This is candidate evidence, not independently qualified merge-main.
