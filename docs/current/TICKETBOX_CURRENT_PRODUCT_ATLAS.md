...
| Income plan revision → forecast → actual edit recovery | Typed immutable revisions preserve monthly history; one query owner supplies whole-month estimates and scheduled-through-today values. API/Web/Android and recycle restore carry the declared month. The real Android editor saves original month, amount, binding, key and OCC to Room before dispatch; recovery remains readable when the management GET fails. Both global sync entrances require that recovery owner, and unsupported old intents remain preserved without futile retry. The protocol epoch and early rejection protect old clients from a schema-level 422. Direct-first edit and Android active-head totals retire. | Candidate `4fcaaeab` passed CI/CodeQL and the real Income Room/UI case; Connected still failed at the unchanged Debt Save case. Subsequent formal P1 protocol and P2 recovery fixes require their own cloud qualification; review threads remain unresolved. Real-router protocol regression passes 16 short database-free cases. See `TICKETBOX_INCOME_PLAN_CONTRACT.md`; no income-accounting subsystem or full RC claim. |
