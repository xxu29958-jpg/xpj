...
| Manual expense entry on Web | `STRONG_SLICE`, merged in #359 at `6829b613` | `create_manual_expense` → native Web form, API and Android manual-entry sheet/Outbox | Native entry, member/viewer admission, currency parsing, device-scoped replay and pending-FX continuation are qualified on candidate and exact merge-main cloud results. Refresh/session-expiry draft continuity remains a required completion task |
| External-debt context: remember why this obligation exists | `STRONG_SLICE`, merged in #361 at `1b0a7631`; merged-main qualification open | Debt create/query owners → Web and Android entry/detail | Optional context now persists through the shared create fingerprint and canonical responses, appears in both clients, and survives ordinary rejected submissions. The discarded Web field is retired. Final candidate passed; merged-main real-db 2/2 hit its 12-minute job cap in both attempts of CI `33967598659`. No test failure was reported, but canceled execution is not qualification. Investigate bounded cloud shard capacity; no further blind reruns or local heavy tests |
| Android external-debt create recovery | `PARTIAL`, not Outbox-enabled | `DebtRepository.createDebt` → guarded direct API; `DebtDraftUi` is in memory | Current source creates a fresh idempotency key per invocation and has no durable create intent. Qualify response-loss retry identity and process/session/ledger transitions before claiming durable recovery or offline creation. Revisit after the context slice; do not infer coverage from other Debt mutations |
...
### External-debt context — implemented and merged; final main gate open
...
Desktop first-use explanation and expired-code recovery links remain companion gaps after this identity slice. Web manual entry's native command consumer is qualified as described above; durable draft continuation is still incomplete. Installation currency adoption remains a Desktop-only ceremony. These facts do not authorize reopening installer, account recovery, upgrades or other Windows HOLD work.
...
The wave is now in construction through #362. Its first task aligns Web and Android genuine empty receipt states with quiet static receipt/coral-side-clip art, neutral state copy and useful native actions. Web removes zero-total filters and row/bulk controls with no rows; filtered-empty results retain the real return path. Android Inbox/Ledger consumers move off the old empty mascot wrapper, without altering loading, role, sync or publication owners. Candidate `d743da15` passed its three cloud workflows and bounded review; the three actual Compose preview artifacts were retrieved and inspected. Current Web 360/768/1440 previews, viewer/filtered states and keyboard focus were inspected. Actual custom-background inspection found a transparent task surface interfering with readability; the existing semantic solid surface is restored, with an observed browser RED/GREEN. Final-head qualification for that CSS correction remains open. Evidence is in `docs/superpowers/plans/2026-09-05-consumer-art-convenience.md`. This task is not the whole five-domain visual system or durable-draft completion.
