...
Owner Console now links to the canonical Web business recycle journey, using the actual business session and ledger permissions. Its narrower duplicate query/restore implementation is physically retired. Each restored entity retains its existing command owner; ledger governance restore remains on the Owner ledger page.
...
Owner discipline, September 6: before and after every business-semantics or owner change, close its impact across all entries, consumers, former success exits, persistence/protocol/recovery paths and direct verification producers. An unaffected claim needs source or execution evidence; unknown impact remains open. Keep the compact before/after evidence with the slice contract. For work already underway when this ruling arrived, reconstruct the actual baseline and compare the current implementation explicitly; do not claim the check happened before the original edit.
...
## 8. Remaining delivery checklist toward the full Internal Beta RC

This is the finite delivery view of the same Goal and Product/G2 contracts, not a smaller replacement goal or a new authority. An existing capability that needs integrated qualification is not labeled a missing feature. Previously closed slices remain closed unless current changed consumers or an executable counterexample reopen them. Counts of PRs, tests or screenshots are not a completion percentage or a duration estimate.
...
The rows record current dispositions against the original bounded source findings; closed slices are not outstanding feature work.
...
**Durable upload continuity, active #385.** Accepted original files/order/identity persist through the bound Room Outbox before Activity handoff acknowledgement; the sole dispatcher and backend atomic receipt retain original retry and capacity-tail semantics. Source 17824a83 passed CI/CodeQL/Connected and its missing-original/permanent-Retry pair was formally resolved/read back, joining the resolved task-input pair. Three later P2s cover restart-orphaned task continuation, unaccepted staging reclamation and Sync Status's alternate per-row Drop. Test-first 7628a188 reproduced two real-PG, one JVM and four Connected failures. Source 1975a0b9 readmits only the validated original orphan through existing capacity/row guards, cleans proven unreferenced failed prefixes, and retires the alternate Drop surface/callback exits. Fresh final qualification and resolution of those three threads remain pending. [The upload contract](TICKETBOX_UPLOAD_INTENT_CONTINUITY_CONTRACT.md) retains all before/after impact, failed heads and retirement evidence. Actual OS process interruption and the full capture task remain separate acceptance claims.

Existing capture → confirm → search/original/correction and budget/category goal → confirmed expense → progress/report/data-health chains are **EXISTS, integrated qualification pending**. They require ordinary member/viewer and cross-client task rehearsal, not duplicate owners. Existing debt plans and background tasks likewise need integrated qualification. Dormant public-admin APIs alone do not authorize a new UI.

**Next bounded capability counterexample: debt-bill image preparation across a household switch.** Both real launchers suspend preparation before a request that currently binds the then-active household. The current HTTP response guard does not cover that earlier interval. Cloud source `4813e4c6` executed 2,244 JVM cases with exactly this new counterexample failing; XML confirms the original bytes reached a request bound to the new household. Candidate `796dc10e` carries one original attempt through preparation, bound request and result; its actual Android-fast job `101672987844` passed compilation, JVM, Lint and Detekt. A repository-weight failure in the direct test producer is corrected by retiring its one-use fake factory, retaining all 35 cases and 108 assertion lines. Combined source `77c1918e` passed CI `34102840809`, CodeQL `34102840765` and Connected `34102840555`. Combined source `44c6caaa` also passed CI `34106951073`, CodeQL `34106951505` and Connected `34106951135`. Capture `1975a0b9` is now integrated; its latest recovery fixes require fresh combined-source qualification and both protected merges remain pending. [The debt-bill binding contract](TICKETBOX_DEBT_BILL_BINDING_CONTRACT.md) maps both entrances, the unique recognition/creation owners, old result exits and affected producers. This remains in Relationships/cross-client continuity; it does not change submitted-intent semantics or open a draft-storage/Windows lifecycle project.

The active rows remain work; closed rows retain their bounded qualification. Each eventual candidate and protected merge-main qualifies independently. #372 stays deferred while substantive capability gaps remain.
...
**Explicit HOLD, excluded from this delivery:** repair, preserved reinstall, complete uninstall, upgrade/downgrade, complete backup/restore and Cut C/D/E. Fresh G2 remains CLOSED. Only the Goal's executable current-product counterexample rule can reopen the minimum necessary host work; final product qualification must not silently claim held lifecycle actions.
