...
| Income plan revision → forecast → actual edit recovery | Server-declared months, immutable/unknown history, legacy scheduled reads and original offline commands retain distinct meanings. Active Android/Web labels now describe plans and estimates. | **Active #377.** `ce047e2c` passed all three gates and its formal threads were resolved/read back. Current `ff060dc3` additionally corrects the non-inline copy finding; final candidate CI and protected-main qualification remain. See `TICKETBOX_INCOME_PLAN_CONTRACT.md`; plans are not confirmed income accounting. |
