...
These are current call-chain observations, not executed failures or completed fixes.
...
| --- | --- | --- |
| Planning recurring occurrence → discretionary Web/API | Every active baseline and all confirmed spending are subtracted, with no period/payment relation. Explicitly associate the same monthly bill and assert that only outstanding commitments remain reserved. | Admitted for construction; route-level RED and implementation pending. |
