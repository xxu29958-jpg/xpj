...
| --- | --- | --- |
| Desktop first use → local device authorization → unknown-result continuation | Healthy local installations expose the existing Owner code task without requiring a phone URL. The actual entry explains local data and Owner/member/invitation meaning. Controller projects an unspent pending attempt without exposing or spending its proof; UI retains entered codes, explains original-code retry and waits for Controller expiry before offering a fresh attempt. | Candidate implementation. Three short Controller/real Edge cases reproduced missing entry and continuation RED; the existing phone-gated device action also reproduced RED. Corrected Controller, UI and related regression coverage pass 82 cases in about four seconds. Actual 820-pixel Edge frame inspected and narrow layout exercised. See `TICKETBOX_DESKTOP_FIRST_USE_CONTRACT.md`. Backend identity ownership and WinCred/proof lifetime remain unchanged; no live ceremony or final RC claim. |
