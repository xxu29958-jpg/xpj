...
| Persistence/recovery | Existing flat UpdateGoal payload and Room schema remain. Unreadable targets fail visibly; bounded transient failures offer same-key retry, terminal refusals offer local withdrawal. Existing scheduler/drain/status/retention owners remain; Room reopen proof is not OS process-death proof |
