...
### dac33fd2 unused status surfaces: before impact

The shared encoder did not satisfy Detekt: Android fast `101620044575` still reports LargeClass. CI `34082384265` failed only the Android lane; CodeQL `34082384241` and Connected `34082384272` succeeded. The next reduction must remove actual duplicate responsibility.

Repository-wide consumer search finds no production caller or callable reference to `OutboxRepository.observeQueueDepth`, `observeConflicts` or `observeFailed`. The actual global status VM uses `observeStatus`, which already queries the same three DAO producers under one captured binding. Only two repository tests call the old depth facade. They exercise same-identity route change and concurrent binding invalidation; migrate those two reads to `observeStatus().map { it.queueDepth }`, retaining the actual leases, transitions, deferred barriers and value assertions. Remove all three obsolete repository methods and update the worker/rule/reaper comments to the actual consumer. The DAO queries, unified status producer, active-by-type product observations, Room schema, protocol, original intent encoding, dispatch, recovery, expiry and file cleanup remain at their current owners. No production entry is redirected and no new bridge or subscription owner is added. Exact cloud Detekt/JVM/Connected will qualify the physical retirement.

After impact: the three methods and stale source references are removed. Both direct transition tests now observe the actual status producer; all their scheduling barriers and original assertions remain. The existing aggregated-status test and all current product callers are unchanged. No file split, suppression, threshold change or new abstraction is introduced. Candidate compilation and execution are still pending.

The pinned CLI runs a single-source syntax analysis in under two seconds with a 20-second process ceiling; it reported 606 class LOC after those deletions. The same exhaustive call-site search also proves the old `pauseForBindingTransition` facade unused: current session producers call the real `withBindingTransition` owner directly. Retire that facade and its obsolete comment, remove the one-line raw-binding alias in favor of the existing provider, and return the existing clear transaction's deletion count directly instead of maintaining a mutable outer latch. All binding leases, canonicalization, completion/cleanup order and exceptions stay unchanged. These are physical retirement and state simplification; no line compression, class/file split, rule waiver or local project build is involved.


## Facts integration before-impact boundary

This local merge preparation combines Capture e86e9cd42ffc8f3b47efdb956f051463956d2faa with Facts candidate ec1578c8f5038c7e1a35b69e213448438d952161. Capture is exact CI/CodeQL/Connected qualified; Facts is still awaiting its own cloud result. Preparation is not protected-main integration or qualification. The root controller is the only writer.

- Entrances: original screenshot batches, first correction Save, explicit correction/refund/other-command Retry and Drop, both global status entrances and the outer fact route. All use their existing domain command owner and the shared Outbox.
- Consumers: upload Inbox observation/file previews, correction fact and stream recovery, both status surfaces, retained debt observations, advice replay notification and worker drain. The merged status observer must join uploads to the same current binding that fences corrections and debts.
- Old success exits: never replace a correction's delivered-but-refresh-required receipt with bare Done, never discard the upload receipt, never adopt independent old-ledger observations, and never navigate recovery through the nested shell graph. Retire the obsolete unproven-correction deletion facade while preserving the current bound discard boundary.
- Persistence/protocol/recovery: one DAO markDone persists both receiptJson and correction refresh error in the same update. Correction, refund and upload originals all remain excluded from fresh-token substitution. Shared recovery schedules only an actually changed, non-expired command and retains Capture file-deletion notification outside leases. Upload batches retain the existing single/batch intent encoder, file lock, binding/dispatch leases and original-key reattachment. No schema or API change is required by the integration itself.
- Direct verification producers: preserve all Facts JVM/Room/stream/adoption/recovery assertions, Capture real WorkManager scheduling and upload Room/process/receipt/file-GC assertions, both navigation controls and the original fake DAO semantics. Merge the Compose rule retirement with Capture's upload navigation oracle. Preserve the proven invitation test scope cleanup and Capture launch signature. Source checks are seconds-scale; all compilation, runtime and device qualification remain cloud-only.


The resolved integration's pinned PSI count is 660 executable class lines in OutboxRepository, above the existing 600 boundary. Source comparison finds duplicated Conflict/Failed recovery algorithms and two otherwise identical fresh-token DAO updates. Consolidate their actual status-guarded update, expiry decision and notification owner, retaining both existing typed public recovery entrances. Retire both duplicate DAO names and their duplicate Fake implementations; migrate the three real-Room direct producers to the single expected-status query without removing assertions. Correction/refund/upload exclusions, owner/ledger matching, key rotation only for key-bearing mutable commands, original Retry, stale-status no-op, expiry and outside-lease file notification remain obligations of existing tests. Also reuse the existing withActiveBinding boundary for bound queue operations by passing its already validated canonical binding to the callback; migrate its one function-valued cache consumer explicitly. Its upload file callback and binding transition tests remain consumers, not presumed unaffected. No new owner, mutex, status, threshold, baseline or suppression is introduced.


## Facts integration after-impact closure

All ten textual conflicts are resolved. The existing upload drain loop and its one settlement handler remain; Success forwards both named cacheRefreshVersion and receiptJson into the same DAO Done update. Four positional upload receipt producers now name receiptJson, preventing a same-typed argument from silently becoming lastError. The real and Fake fresh-token update both reject correction, refund and upload originals and require the supplied status to be Conflict or Failed. Their two duplicate query implementations are physically retired. Existing typed Conflict/Failed API entrances now delegate to one recovery decision under the existing lease; expiry, original Retry, stale-status no-op, conditional wakeup and post-lease file cleanup retain their semantics. The existing withActiveBinding helper supplies the canonical binding already validated inside its lease to six queue consumers; the function-valued cache callback is explicitly adapted and the upload timestamp callback remains unchanged.

Both status entrances use the outer fact navigator and retain the upload Inbox continuation. Global status still has one combined current-binding observer; upload recovery uses that already scoped status, adding no independent collector. The malformed-refresh barrier, unproven original discard and immutable first correction admission survive. The merged Room fixture retains its real session observation and correction stream race hook; it no longer requires a nonempty queue on every legitimate successor wakeup. Both the Compose field rule and upload navigation oracle survive. Facts 847167f2's two immutable test fixtures are carried without dropping any of its 11 scenarios.

The shared mutation-label map already owns a total enum mapping. Its one-line forwarding function is retired and the screen, Drop dialog and existing label test read that same map directly, retaining labels and Unknown coverage. No duplicate mapping or extra file is introduced.

Pinned Detekt light analysis passes all changed production sources against grayDebug and all changed JVM sources against grayDebugUnitTest. The shared Outbox PSI class count falls from 660 to 596 through retirement of duplicated recovery/binding work, not through a threshold or baseline change. Actual repository-weight function analysis of the four changed shared owner/producer files has no over-80 span or over-15 complexity. Resource XML and diff checks pass. A mixed-source diagnostic initially applied the production baseline to JVM and Connected files; its pre-existing producer findings are not claims of new regressions or executed runtime failures. Main/JVM source checks now use their actual respective baselines. Connected remains the runtime/Room authority for its unchanged existing test producers.

This local integration has not executed Kotlin/Room compilation, JVM or device tests and does not yet qualify PR385, protected main or the whole Goal. Those exact cloud checks follow this candidate; protected-main merge order remains Facts first, with independent main qualification before subsequent slices.


### e2154bbb compile result and upload-card consumer correction

Android-fast job 101637546531 stops in compileGrayDebugKotlin: SyncStatusUploadSection still calls the pre-integration FailedCard onDrop parameter. The common card now consumes SyncStatusActions for original-offset review. This is a missed merge consumer, not a test failure or qualified runtime. Before/after closure checks all three FailedCard occurrences: the declaration, ordinary failed-row caller and upload caller. The upload caller adapts its existing failed/conflict Drop choice into onDropFailed on a copied action set. Its null Retry, immutable row capture, confirmation path, Inbox navigation and original onDropMine/onDropFailed callbacks remain. The ordinary failed-row and refund-review consumers retain their existing action set. No repository, command, protocol or retry semantics change. Compilation and the existing upload navigation/confirmation Connected oracle must execute on the corrected candidate.

### 79ec7b8a test-construction impact closure

Android-fast job 101640698773 passes production compilation but stops in compileGrayDebugUnitTestKotlin at four Facts fixture calls that omit Capture's required onRowsDeleted hook. Before correction, a Kotlin PSI call-expression scan of main, JVM and instrumentation sources finds 22 actual OutboxRepository constructions, with exactly these four missing policies: ExpenseCorrectionGlobalRecoveryTest, both ExpenseCorrectionRefreshRecoveryTest fixtures and OutboxCorrectionBindingTest. These fixtures own no upload files; each now explicitly supplies an empty deletion callback. Their original clocks, live binding streams, queue rows, recovery actions and assertions are unchanged. Production AppContainer keeps the real file-cleanup hook, and the production constructor remains mandatory. No persistence, command, protocol, recovery or success-exit semantics change. Repeating the same structural scan must find all 22 constructions with explicit policies; the actual JVM and Connected producers still require final-candidate cloud execution.

Connected job 101640700771 independently stops at instrumentation-source compilation before executing any device case: two Facts screen callers omit onOpenInbox and FactEntryNavigationHarness omits MainFeatureRepositories.uploadIntents. Both screen-only Facts fixtures now name their unused Inbox callbacks, preserving the existing fact-opening and stale-binding discard oracles. The navigation harness receives a real UploadIntentRepository from its existing Room fixture, using that same session/provider, outbox, adapters and settings; it does not substitute another session or fake action owner. The fixture exercises only fact navigation and creates no upload files. All MainFeatureRepositories, SyncStatusScreen and SyncStatusScreenContent call expressions are checked for the required inputs; production callers retain their real Inbox navigation and upload owner. No default callback or nullable dependency is added to production to conceal these missed consumers.

After correction, the pinned PSI scan finds all 35 actual calls across the four affected APIs with explicit required inputs, including all 22 Outbox constructions. Changed JVM sources pass their actual unit-test Detekt baseline; all seven changed Kotlin producers pass the repository's function-span/complexity measurement and diff checks. These are source results only; final-candidate JVM and Connected execution remain pending.

### 408e8eaf storage-transition interface correction

Actual Android-fast job 101649458371 compiles production/JVM sources and executes 2,243 JVM cases with zero failures, errors or skips (386 XML suites, artifact 10007702888). It then fails the real type-aware Detekt gate because the newly generalized requeueWithFreshToken has six scalar parameters. The earlier light-source scan did not detect this issue and is not equivalent to the actual build gate.

Before-impact: the only production caller is the already consolidated resolveStatus owner, reached by typed Conflict/Failed recovery. The Room query must atomically compare row identity, owner, ledger and the requested prior status while preserving OCC/key replacement and immutable correction/refund/upload exclusions. Removing one condition or moving it to an unprotected pre-read would weaken this contract. A generic arbitrary-status DAO is not needed by the two real transitions. Keep the unified expiry/scheduling/deletion decision in OutboxRepository, but restore explicit Conflict-to-Pending and Failed-to-Pending storage operations with five actual inputs each. These are two conditional SQL transitions, not two recovery decision owners. Both retain all three immutable-type exclusions from the integrated Facts/Capture source. The Fake implementation and all three direct Room calls migrate with them; original assertions, retry budget, null-key behavior, blocksFollowing and stale-status no-op remain. No parameter bag, extra transaction, default, suppression or threshold change is introduced. The generic six-argument DAO entrance is retired.

After-impact: the one private recovery owner chooses the corresponding status-specific DAO operation; expiry, Drop and successor scheduling stay in that owner. Both SQL statements and both Fake overrides preserve identity/status predicates and all three immutable-type exclusions. Three direct real-Room calls retain their original expected affected counts and row assertions. A source-query comparison executes 320 small synthetic SQLite controls spanning both transitions, current states, immutable types, matching/foreign owner and ledger, and present/null keys; affected count and complete stored row match the previous query in every case. This is not Room runtime evidence. Pinned PSI measures OutboxRepository at 598 class lines and resolveStatus at 22, preserving the actual consolidation. Main/unit light Detekt and diff checks pass; no build threshold, baseline or suppression changed. Actual type-aware Detekt, Room and consumer tests remain final-candidate cloud gates.

The next normal main integration is restricted to qualified-source governance changes from 61f3d69c, now squash main d5c0da54 with the same tree. A clean preview names only the six maintenance/help/current-index/producer files already covered by that contract; upload/Facts sources are unchanged by that integration. Main's independent qualification is still running, so this permits candidate construction and testing only, not the next protected merge.

### af93dd5f complete-candidate integration checks and retirement

Cloud Android-fast 101654498042 passes the production Detekt gate, then rejects the merged OutboxStatusViewModelTest class at 12 functions. Backend contracts 101654498028 independently reports only files_over_800 growth: PendingMutationDao is 813 lines against main's 795. Its other release-audit gates pass. The previously used last-edit source scope missed this merged producer. A full current-candidate light scan against main covers all 47 changed production and 41 changed JVM files; production has zero findings and JVM reproduces exactly that test-class finding. Actual Connected 34094342553 succeeds at af93dd5f; these facts do not qualify its failed CI.

Before-impact: assertOriginalAdjustmentContext is a one-call test wrapper around eight existing assertions. Inline those assertions at their only call, preserving all seven scenarios and their scope cleanup; retire the wrapper and its three now-unused type imports without moving tests to another class/file. For the storage owner, tracked-source search finds the old unguarded refreshToken writer only in PendingMutationDao and FakePendingMutationDao declarations, with no invocation or function reference. The real recovery callers already use status-guarded transitions or bound target propagation. Retire both dead declarations together; no persisted schema, protocol, caller, recovery result or migration changes. Replace the old recovery comment's review-history narrative with its current atomic identity/status checks, replacement-command semantics, null-key behavior and immutable correction/refund/upload prohibition. No SQL condition, baseline, threshold or suppression is weakened.

After-impact: both dead writer declarations and the one-call assertion wrapper are absent. All seven test names and every original assertion line remain, verified as an unordered multiset after excluding only the retired wrapper invocation. PendingMutationDao now has 792 physical lines. Repeating the entire current-candidate scan passes all 47 changed production and 41 changed JVM files against their actual respective baselines, with zero findings. The existing SQL transitions, binding/scheduler/file owners and real-Room assertions remain unchanged by this retirement. Diff checks pass. These bounded source results do not substitute for final-candidate type-aware compilation, executed tests or full repository-weight qualification.

### 61674df7 annotation retirement closure

Android-fast job 101660907602 passes production and JVM Detekt, then Android Lint rejects the old StringRes function annotation left attached to the shared mutation-label Map after the forwarding function was retired. Remove that invalid annotation and its now-unused import. The total resource map, three screen reads, Drop-dialog read and existing total-enum/Unknown assertions remain byte-for-byte unchanged; no business entry, persistence, protocol, recovery or success exit changes. This completes the same facade retirement without restoring a wrapper or adding a suppression. Connected execution 101660916585 succeeded on the failed candidate; its CI remains unqualified. The corrected source requires fresh cloud Lint and exact-candidate gates.

### Formal P2: committed receipt with an unsubmitted enrichment task

Review thread `PRRT_kwDOS5LrfM6f1H8O` identifies the interval left open by the earlier read-back correction: the commit can succeed while its acknowledgement and immediate verification read are both unavailable. The request correctly preserves uncertainty, but a later keyed HIT currently returns before submitting the original queued task. The task row persists; its original execution payload currently exists only in PreparedBackgroundTask memory. Startup failure classification is not execution recovery. The earlier blanket assertion that HIT must never submit is therefore too strong and is superseded for an original task that remains queued.

Before-impact closure: the entrance is keyed app upload replay; ordinary app/Web/UploadLink preparation and original commit are direct producers of the same enrichment task. Consumers are Android/Web task polling, pending suggestion/OCC application and existing task cancellation/status. Recovery must use the persisted original task identity, timezone and expected row version, never the expense's newer revision or ambient request defaults; receipt, expense and attachment identities remain immutable. Existing queued-to-running CAS must keep duplicate wakeups from executing twice, and an executor-refusal publication must not overwrite a concurrently claimed/completed task. Terminal task outcomes are not restarted. Durable original task input and its application migration are necessary; other task families and startup orphan policy gain no automatic replay authority. The existing lost-commit-ack test now independently varies read-back availability and requires the original submission after a HIT when immediate verification was unavailable. This test-first extension has not yet run in cloud; production is unchanged at this step.
