...
Formal review of `4fcaaeab` admitted a second P1: the new required month had retained the old runtime protocol epoch. An independent review confirmed that a version bump alone would still let the old interceptor send an unversioned request. Real-router tests reproduced the old/headerless 422 path before the fix; all 16 isolated protocol cases now pass with database connections prohibited. Current-client negotiation refusal and both global sync recovery corrections have targeted Android tests awaiting cloud execution. The two formal recovery P2s and both P1 threads remain open until supported formal closure; a local fix or an outdated diff is not a resolved review.
