...
Status: revision/query owner, month-bearing API/Web/recycle/Android consumers and durable edit recovery are implemented for candidate qualification. Five pure forecast/template cases pass locally; PostgreSQL, Android unit and actual Room/UI continuity tests are added but not yet executed. No runtime closure or full RC claim.

Bounded main review on `92071386` admitted one P2: the actual Android editor defaulted to the device month, which can disagree with the server accounting month. The real screen now passes its confirmed forecast month into the edit session; create also waits for that confirmed month. Original open/submitted intent stays frozen. The first cloud candidate exposed stale migration-head assertions, an undated monthly Advisor fixture and two incomplete Android test constructors; these are migrated without weakening their original assertions. Runtime qualification remains pending on the repaired candidate.
