...
## Native Edge lifetime observation before choosing a correction

At 0d28c801, Desktop job 101641809866 executes 472 passed / 1 failed / 10 skipped. The sole failure is the existing production Edge lifetime test: the launched process remains alive beyond its ten-second close wait. The test never independently observes a native window or whether its page loaded and called window.close, so that timeout does not establish a browser-process defect or justify a larger timeout. The decision blocked is whether to correct test timing/page semantics or the production window owner. The smallest missing facts are the visible window's process identity and loaded/closing stages when the process stays alive.

The disposable cloud producer retains the actual production launcher, flags, unique profile, two-second page close action and ten-second exit bound. An independent Win32 EnumWindows / IsWindowVisible / GetWindowThreadProcessId observer records only counts and fixed synthetic title stages. Its negative control uses the existing never-closing page through the same launcher and must observe that page under the owned process before the host closes it; cleanup runs even when assertions fail. No credentials, URLs, arguments or unrelated window titles are emitted. Microsoft owns these native observation semantics: [EnumWindows](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-enumwindows), [GetWindowThreadProcessId](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getwindowthreadprocessid). The actual cloud Edge version must be checked before attributing a browser-version-specific behavior.

If the page never becomes visible or never reaches the close stage, inspect that earlier launch/page boundary. If a visible probe moves to another process, the launch handle is not the window owner. If the owned visible window disappears while the original process remains, process liveness is not sufficient for the Manager's visible-window decision. If both end correctly, this run proves only that exact successful observation and does not retroactively explain 0d28c801's timeout. This change is observation and stronger assertions, not a production fix. All CSV, session, privacy, lifecycle and protocol owners remain unchanged; only the two direct native producer cases and the existing test support module change. No local native/Edge execution or Windows lifecycle work is opened.

At 4305d731a42b662ea03ce7df32c9a5b98ccb25ad, CI 34092912341, CodeQL 34092912305 and Connected 34092912291 all succeed. Actual Desktop job 101650308216 executes 473 passed / 10 skipped, then the explicit Web runtime group passes 3 cases. Both Windows-only native lifetime producers now observe their real windows and satisfy their original process-exit conditions. The earlier 0d28c801 timeout remains unexplained; no browser-specific cause, platform fix or timeout extension is claimed. Source AST/Ruff/diff checks also passed; unchanged existing long embedded-JavaScript producers are not newly introduced complexity findings.

Before/after the next integration, only the six already qualified governance maintenance/help/current-index/producer paths should change. Source 61f3d69c and protected main d5c0da54 have the identical tree. The normal source-branch merge preview is clean; when the equivalent squash-main history produces textual conflicts, resolve those paths to the qualified governance source and require the entire resulting tree to equal that source-branch preview. CSV commands, saved-batch queries, counts, leases, idempotency, native continuation and their direct tests must remain identical. Main independent qualification is still running; final combined-candidate gates and formal closure precede the next protected merge.
