...
| Accepted command publication | Durably admitted PATCH, confirm, reject, mark-not-duplicate, retry-OCR, recognize-text and offset create/void keep known server success when their rebuildable publication fails. Reuse Done, the original key/body and existing refresh-version marker. Close sender, subsequent OCC, scoped reads, marker acknowledgement/retention and reopened SyncStatus together; never decode these commands as Correction payloads. Confirmed/offset repair requires the adopted root and complete stream projection before acknowledgement. Cancellation and request refusals retain their existing semantics. Manual creation uses a separate local-reference/receipt adoption owner and is unchanged by this publisher; its existing recovery gap remains in full-Goal delivery |
