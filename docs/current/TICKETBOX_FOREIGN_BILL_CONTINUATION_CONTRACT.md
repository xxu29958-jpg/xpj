...
| Accepted command publication | Durably admitted PATCH, confirm, reject, mark-not-duplicate, retry-OCR, recognize-text and offset create/void keep known server success when their rebuildable publication fails. Reuse Done, the original key/body and existing refresh-version marker. Retain the accepted Expense ID in that same command's existing receipt: local-reference refresh, Fact, subsequent numeric admission and read acknowledgement must survive retirement of the separate Create receipt and disposable identity cache. Close sender, subsequent OCC, scoped reads, marker acknowledgement/retention and reopened SyncStatus together; never decode these commands as Correction payloads. Confirmed/offset repair requires the adopted root and complete stream projection before acknowledgement. Cancellation and request refusals retain their existing semantics. Manual creation uses a separate local-reference/receipt adoption owner; its existing recovery gap remains in full-Goal delivery |
| Remaining direct submission owner | FIX/NEXT: real pending-editor and review actions still try online requests before durable admission. Their local-publication failure can lose the unpersisted original key; the current Outbox recovery fix does not close those paths. Continue their admission/retirement in the same Goal's Capture package immediately after this slice; do not hide the gap with an IOException conversion or claim all financial submissions complete |
| Product consumers | Web/Android pending queue, bill editor/detail, import result and data-health entries identify blocked bills and return to review. Task polling returning no task must leave explicit latest-bill review reachable while preserving unsaved fields and OCC. A changed original currency must not hide Web revision recovery. Android offers explicit keep/replace consent for a dirty draft; it must not require a stale-version save before reloading. Invalid daily quote times/timezone must not stop the existing accepted-bill continuation loop; Owner FX status must show that configuration error even while continuation is running, without promising scheduled quote recovery |
