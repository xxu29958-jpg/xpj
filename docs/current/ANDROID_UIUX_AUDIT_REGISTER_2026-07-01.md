...
| Unit tests | Today next-action priority, Insights goal header-state decisions, Pending overview, row-density token decisions, tag-filtered dashboard tab mapping, primary-stats refresh completion, and report overview-before-goals refresh completion have focused JVM tests. The Android test-count baseline is `1243`. | Add focused tests only where state authority or UI decisions are risky. |
