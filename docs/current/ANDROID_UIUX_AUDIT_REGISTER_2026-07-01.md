...
| --- | --- | --- | --- |
| SET-001 | Connection | Needs QA | Connection now shows the bound ledger address, separates confirmed service state from cached state, uses resource-backed role/scope labels, and avoids exposing raw storage/role tokens. Final true-device verification is still required, especially under VPN/offline transitions. |
...
| Unit tests | Today next-action priority, Insights goal header-state decisions, Pending overview, row-density token decisions, tag-filtered dashboard tab mapping, primary-stats refresh completion, report overview-before-goals refresh completion, and recent-window trend comparison have focused JVM tests. The Android test-count baseline is `1245`. | Add focused tests only where state authority or UI decisions are risky. |
