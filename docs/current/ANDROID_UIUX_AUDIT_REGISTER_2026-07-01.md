...
| Gradle compile/detekt | Current Settings secondary slice passes `detektGrayDebug` and `assembleGrayDebug`; detekt still reports the existing compiler-analysis warning but 0 findings. | Rerun flavor-qualified gates after code changes. |
...
- User-observed secondary-page drift now includes About, Insight Tag/Budget/Goal
  drill pages, and Pending real-item rows. Treat those as root-cause IA work,
