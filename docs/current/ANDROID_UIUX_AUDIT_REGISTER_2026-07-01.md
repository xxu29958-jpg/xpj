...
| Unit tests | Today next-action priority, Today background-refresh authority tone, readable refresh-indicator release, Insights goal header-state decisions, Pending overview, row-density token decisions, tag-filtered dashboard tab mapping, primary-stats refresh completion, report overview-before-goals refresh completion, recent-window trend comparison, Connection fresh-settings gating, Background Tasks label mapping, and Recycle Bin summary/failure-state handling have focused JVM tests. The Android test-count baseline is `1267`. | Add focused tests only where state authority or UI decisions are risky. |
