...
| AIA-010 | Settings secondaries | In progress | P1 | Secondary pages are uneven. Sync status has improved, and the Recycle Bin page now has a compact overview/list model with explicit read-only, empty, loading, failed, restoring, and backend-count states. Other pages still need page-by-page state parity. User follow-up: second-level surfaces must match the five root tabs and avoid isolated rounded-card language. | Each secondary page has consistent title, back, explanation, loading, empty, error, cached/offline, read-only, success, destructive states, density, and component language. |
...
| FUNC-SET-001 | Settings secondary state parity | Registered | Secondary pages do not all expose the same loading, empty, error, read-only, cached, and destructive states. The Recycle Bin page now closes its code-level parity pass, but other secondaries remain open. | Close one page at a time and update the Secondary Settings Register. |
...
| S6 Settings governance | In progress | `b61dbedd` plus Settings root, My Devices, Family Members, Connection, Join Family, Category Rules, Background Tasks, and Recycle Bin secondary slices | Root governance and secondary state parity. |
