...
| AIA-025 | Refresh latency | Needs QA | P0 | User follow-up: frontend refresh/loading feels too slow. The current slice ends the root loading state as soon as authoritative monthly stats return, moves lifestyle/confirmed-sync work behind the primary result, and lets Insights report overview/trend release before Goals finishes. True-device timing is still pending. | Pull-to-refresh keeps existing content readable, does not block the whole page on non-primary modules, and never claims backend freshness before the primary authoritative response arrives. |
...
| FUNC-REFRESH-001 | Refresh loading latency | Needs QA | Root-page refresh can feel slow when the page-level loading flag waits for secondary work after the primary facts are available. | Today/Insights end primary refresh on authoritative monthly stats; lifestyle stats and confirmed-cache sync continue behind the primary result, and report goals no longer hold the reports/trend loading state after overview arrives. |
