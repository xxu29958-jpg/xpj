...
| AIA-023 | About / product trust page | Registered | P1 | User observation calls out "About Ticketbox" as part of the secondary-page mismatch. The About page still needs release-facing copy, support/build information hierarchy, and visual alignment with Settings. | About reads as a polished product trust page, not a diagnostics dump; version/build/support/legal rows are resource-backed, compact, and consistent with Settings secondaries. |
| AIA-024 | Edit / confirm detail | Needs QA | P0 | User screenshot `Screenshot_2026-07-01-23-48-19-093_com.ticketbox.jpg` showed the amount/currency edit surface with the keyboard open: the bottom action area became a large rounded block, actions formed a heavy 2x2 grid, and the amount section still used old card language. The current slice collapses keyboard-visible actions into a compact strip and moves amount placeholders into resources. | The edit/confirm detail page uses the same secondary-workbench language as Pending and Ledger, keeps keyboard actions thin and thumb-reachable, avoids hardcoded placeholders, and does not hide the form behind a large action card. |
...
| SET-014 | About | Registered | Version/build/support copy stays product-level, support/legal rows are structured, and diagnostics are not exposed in release-facing text. |
...
| FUNC-EDIT-001 | Edit keyboard action density | Needs QA | The receipt edit page is a shared path for Pending review, Ledger edits, and manual confirmation. With the keyboard open, a large action block can consume too much of the first viewport. | Keyboard-visible actions now collapse into a compact action strip; primary and destructive actions remain clear; validation/status copy stays visible without becoming a large card. |
...
| Gradle compile/detekt | Current edit-density slice passes `detektGrayDebug` and `assembleGrayDebug`; detekt still reports the existing compiler-analysis warning but 0 findings. | Rerun flavor-qualified gates after code changes. |
