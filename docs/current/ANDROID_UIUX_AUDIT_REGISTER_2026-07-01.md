...
| AIA-025 | Refresh latency | Needs QA | P0 | User follow-up: frontend refresh/loading feels too slow. The current slice ends the root loading state as soon as authoritative monthly stats return, moves lifestyle/confirmed-sync work behind the primary result, and loads Reports overview/goals concurrently. True-device timing is still pending. | Pull-to-refresh keeps existing content readable, does not block the whole page on non-primary modules, and never claims backend freshness before the primary authoritative response arrives. |
...
| Unit tests | Today next-action priority, Insights goal header-state decisions, Pending overview, row-density token decisions, tag-filtered dashboard tab mapping, and primary-stats refresh completion have focused JVM tests. The Android test-count baseline is `1237`. | Add focused tests only where state authority or UI decisions are risky. |
