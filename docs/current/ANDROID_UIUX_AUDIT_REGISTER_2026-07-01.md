...
| Unit tests | Today next-action priority, Insights goal header-state decisions, Pending overview, row-density token decisions, tag-filtered dashboard tab mapping, primary-stats refresh completion, report overview-before-goals refresh completion, recent-window trend comparison, and Connection fresh-settings gating have focused JVM tests. The Android test-count baseline is `1247`. | Add focused tests only where state authority or UI decisions are risky. |
