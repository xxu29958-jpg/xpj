...
| AIA-025 | Refresh latency | Needs QA | P0 | User follow-up: frontend refresh/loading feels too slow. The current slice prioritizes authoritative monthly stats before supplemental Insights modules, starts Today's month refresh before Pending, and stops Today's pull indicator from being held by a background Pending refresh when content is already visible. True-device timing is still pending. | Pull-to-refresh keeps existing content readable, does not block the whole page on non-primary modules, and never claims backend freshness before the primary authoritative response arrives. |
...
| FUNC-REFRESH-001 | Refresh loading latency | Needs QA | Root-page refresh can feel slow when secondary requests start before the primary facts or keep the page-level indicator active after readable content already exists. | Today/Insights prioritize authoritative monthly stats before supplemental work; Today's existing Pending content no longer holds the pull indicator, lifestyle stats and confirmed-cache sync continue behind the primary result, and report goals no longer hold the reports/trend loading state after overview arrives. |
