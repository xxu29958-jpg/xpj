...
| FUNC-REFRESH-001 | Refresh loading latency | Needs QA | Root-page and drill-page refresh can feel slow when secondary requests start before the primary facts, keep the page-level indicator active after readable content already exists, or sync confirmed ledgers in too-small pages. | Today/Insights prioritize authoritative monthly stats before supplemental work; Today/Pending/Ledger existing content and readable monthly stats no longer hold the page pull indicator, secondary/drill pages now use shared readable-refresh gating, lifestyle stats and confirmed-cache sync continue behind the primary result, report goals no longer hold the reports/trend loading state after overview arrives, and confirmed-ledger sync requests 200 rows per page. |
