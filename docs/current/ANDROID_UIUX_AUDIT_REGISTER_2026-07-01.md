...
| AIA-004 | Pending | Needs QA | P0 | Pending now has a real queue overview derived from `state.items`; the 2026-07-01 density slice reduced repeated top metrics, compressed the queue overview, and made compact cards scan-first. Broader action/empty/offline QA remains open. | No fake duplicate/ready counts; the user can immediately see ready, missing amount, missing merchant, duplicate, and blocked work. |
