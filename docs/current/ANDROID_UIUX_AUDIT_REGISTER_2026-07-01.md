...
| FUNC-SET-001 | Settings secondary state parity | Registered | Secondary pages do not all expose the same loading, empty, error, read-only, cached, and destructive states. Recycle Bin closes its code-level parity pass, and Appearance/background closes the first-read/resource-language pass, but other secondaries remain open. | Close one page at a time and update the Secondary Settings Register. |
...
| S6 Settings governance | In progress | `b61dbedd` plus Settings root, My Devices, Family Members, Connection, Join Family, Category Rules, Background Tasks, Recycle Bin, and Appearance/background secondary slices | Root governance and secondary state parity. |
...
| Gradle compile/detekt | Current Appearance/background slice passes `compileGrayDebugKotlin`, `detektGrayDebug`, `detektGrayDebugUnitTest`, `testGrayDebugUnitTest`, `lintGrayDebug`, `assertAndroidTestCountEqualsBaseline`, and `assembleGrayDebug`. | Rerun flavor-qualified gates after code changes. |
| Unit tests | Today next-action priority, Today background-refresh authority tone, readable refresh-indicator release, Insights goal header-state decisions, Pending overview, row-density token decisions, tag-filtered dashboard tab mapping, primary-stats refresh completion, report overview-before-goals refresh completion, recent-window trend comparison, Connection fresh-settings gating, Background Tasks label mapping, Recycle Bin summary/failure-state handling, Merchant Management summary handling, and Appearance summary/resource mapping have focused JVM tests. The Android test-count baseline is `1273`. | Add focused tests only where state authority or UI decisions are risky. |
| Lint | Required for broader Android slices. | Run before any final Android batch commit/push. |
| True device | Physical device `5c52fc22` installed the official `com.ticketbox` gray package against `https://api.zen70.cn`. Clean online screenshots `80-insights-budget-after-fix.png` and `81-insights-goal-after-fix.png` confirm Budget shows "未设置预算/未启用" and Goals shows "未设置" for zero enabled goals. Screenshot `83-pending-after-density-fix.png` confirms Pending compact mode shows two queue rows in the first viewport without expanded duplicate notices. Screenshot `90-ledger-density-final-settled.png` confirms Ledger header compaction, compact row meta, full amounts, and bottom-nav clearance on live data. Screenshots `95-settings-root-portrait-after-density.png` and `96-settings-root-settled-after-density.png` confirm Settings initial refreshing state and settled confirmed state. The phone was unplugged before final Connection, My Devices overflow-menu, Family Members, Join Family, Category Rules, Recycle Bin, Appearance/background, Budget secondary, populated Goals, tag-filtered Insights, the new Pending real-item row, edit-detail keyboard, and refresh-latency screenshots could be captured. Current ADB check shows only `emulator-5554`, not a physical device. | Reconnect physical device and capture Connection online/offline/VPN behavior, Settings Devices, Family Members, Join Family bound/cold-start states, Category Rules list/edit states, Recycle Bin empty/populated/read-only states, Appearance/background overview/gallery/crop/preview/custom-image/clear states, Budget not-enabled/active states, populated Goals rows, tag-selected Insights, Pending with real unconfirmed items, edit amount/currency keyboard state, and Today/Insights pull-to-refresh timing before closing SET-001/SET-003/SET-004/SET-005/SET-006/SET-009/SET-013/FUNC-PEND-002/FUNC-REFRESH-001/FUNC-INS-007/FUNC-INS-008/FUNC-INS-009/AIA-024/AIA-025. |
