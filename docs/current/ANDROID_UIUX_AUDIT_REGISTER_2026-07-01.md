...
| AIA-024 | Edit / confirm detail | Needs QA | P0 | User screenshot `Screenshot_2026-07-01-23-48-19-093_com.ticketbox.jpg` showed the amount/currency edit surface with the keyboard open: the bottom action area became a large rounded block, actions formed a heavy 2x2 grid, and the amount section still used old card language. Current slices fix IME-state propagation for both the floating action bar and form content, collapse keyboard-visible actions into a compact strip, move amount input before currency chips, and stop existing receipt edit pages from auto-opening the keyboard while preserving auto-focus for manual entry. | The edit/confirm detail page uses the same secondary-workbench language as Pending and Ledger, keeps keyboard actions thin and thumb-reachable, avoids hardcoded placeholders, and does not hide the form behind a large action card. |
...
| Gradle compile/detekt | Current tag-scope Insights slice passes `detektGrayDebug` and `assembleGrayDebug`; detekt still reports the existing compiler-analysis warning but 0 findings. | Rerun flavor-qualified gates after code changes. |
| Unit tests | Today next-action priority, Insights goal header-state decisions, Pending overview, row-density token decisions, and tag-filtered dashboard tab mapping have focused JVM tests. The Android test-count baseline is `1237`. | Add focused tests only where state authority or UI decisions are risky. |
| Lint | Required for broader Android slices. | Run before any final Android batch commit/push. |
| True device | Physical device `5c52fc22` installed the official `com.ticketbox` gray package against `https://api.zen70.cn`. Clean online screenshots `80-insights-budget-after-fix.png` and `81-insights-goal-after-fix.png` confirm Budget shows "未设置预算/未启用" and Goals shows "未设置" for zero enabled goals. Screenshot `83-pending-after-density-fix.png` confirms Pending compact mode shows two queue rows in the first viewport without expanded duplicate notices. Screenshot `90-ledger-density-final-settled.png` confirms Ledger header compaction, compact row meta, full amounts, and bottom-nav clearance on live data. Screenshots `95-settings-root-portrait-after-density.png` and `96-settings-root-settled-after-density.png` confirm Settings initial refreshing state and settled confirmed state. The phone was unplugged before final Connection, My Devices overflow-menu, Family Members, Join Family, Category Rules, Budget secondary, populated Goals, tag-filtered Insights, and edit-detail keyboard screenshots could be captured. | Reconnect physical device and capture Connection online/offline/VPN behavior, Settings Devices, Family Members, Join Family bound/cold-start states, Category Rules list/edit states, Budget not-enabled/active states, populated Goals rows, tag-selected Insights, and edit amount/currency keyboard state before closing SET-001/SET-003/SET-004/SET-005/SET-006/FUNC-INS-007/FUNC-INS-008/FUNC-INS-009/AIA-024. |
