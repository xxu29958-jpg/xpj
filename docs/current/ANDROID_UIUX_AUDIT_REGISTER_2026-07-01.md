...
| AIA-025 | Refresh latency | Needs QA | P0 | User follow-up: frontend refresh/loading feels too slow. Current slices prioritize authoritative monthly stats before supplemental Insights modules, start Today's month refresh before Pending, stop Today/Pending/Ledger page-level pull indicators from being held while readable content already exists, release Today/Insights pull indicators when existing monthly stats are readable, and raise confirmed-ledger sync page size from 50 to the backend-supported 200. True-device timing is still pending. | Pull-to-refresh keeps existing content readable, does not block the whole page on non-primary modules, reduces avoidable confirmed-ledger round trips, and never claims backend freshness before the primary authoritative response arrives. |
