...
更细的 `v0.4-alpha3-rc1 -> v1.0` 能力任务清单见 `docs/MONARCH_CAPABILITY_ROADMAP.md`。基于当前 `v0.4-beta1` 代码基线继续推进时，以 `docs/POST_BETA_DEVELOPMENT_ROADMAP.md` 作为工程顺序、风险前置项和验收标准。Monarch 只作为成熟产品的信息架构参考，不改变小票夹的隐私隔离、本地优先和人工确认定位。
...
- 更顺滑的图片预览和编辑流程。
- 账单搜索、标签和轻量复盘。
- Android、`/web`、`/owner` 三端 UI/UX 美化统一，保持同一套状态语义、卡片节奏、空状态和错误文案。
...
- 不引入浮点金额。
- 不暴露技术诊断给普通用户。
- 图表库只允许在报表增强阶段经过依赖审计和 ADR 后进入展示层，不能进入后端、统计口径、预算规则或权限过滤。
- 三端视觉统一不能把 `/web` 或 `/owner` 暴露到公网，也不能把运维细节带进普通 Android UI。
...
不要在一个提交里同时混合大规模后端、账本隔离、Android UI、OCR 和文档重构。
