...
`docs/architecture/*`、`docs/DECISIONS/*` 按 `AGENTS.md` 的「控制面与渐进阅读」在相关任务时主动读
...
`.claude/HANDOFF.md` 是跨 session / 跨 AI（codex ↔ claude）的**任务交接合同**真相源。
读写语义以 `AGENTS.md` 为准；claude 经下面的 `@import` 自动加载（`@import` 不展开 markdown
文本路径，所以这里必须显式写）。
