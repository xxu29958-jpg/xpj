...
dependencies {
    ...
    // first run (AOT dexopt of the hot startup path); it self-initialises via its
    // androidx-startup ContentProvider, so no app code is needed. The baselineProfile
    // configuration consumes the profile generated by the :macrobenchmark module.
