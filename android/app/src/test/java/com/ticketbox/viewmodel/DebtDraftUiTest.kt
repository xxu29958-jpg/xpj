...
class DebtDraftUiTest {
    ...
    fun parsedAmountCentsConvertsYuanAndRejectsInvalidInput() {
        ...
        // C07：只能精确落到 minor；尾随零可接受，非零超精度不得静默舍入。
        assertEquals(123L, DebtDraftUi(amountYuanInput = "1.230").parsedAmountCents())
        assertNull(DebtDraftUi(amountYuanInput = "1.005").parsedAmountCents())
