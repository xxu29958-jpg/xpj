...
/**
 * ADR-0042 Slice D-1: [MarkNotDuplicateDispatcher] replay contract.
 *
...
internal class MarkNotDuplicateDispatcherTest : ExpensePendingRepositoryOutboxTestBase() {
    ...
    fun `dispatch replays the row's idempotency key and returns the new row_version`() = runTest {
        val stub = ApiServiceStub(markNotDuplicateResult = ApiResult.Success(successExpenseDto()))

        val result = dispatcherFor(stub).dispatch(markRow(idempotencyKey = "key-abc"))
    ...
    fun `a row with no idempotency key fails loudly instead of silently dropping`() = runTest {
        ...
        val result = dispatcherFor(stub).dispatch(markRow(idempotencyKey = null))
    ...
    fun `409 idempotency_key_in_progress is retried, not dropped`() = runTest {
        ...
        val stub = ApiServiceStub(markNotDuplicateResult = ApiResult.Throw(httpException(409, body)))
