...
/**
 * ADR-0042 Slice D-2: [DeleteCategoryRuleDispatcher] replay contract.
 *
 ...
 */
class DeleteCategoryRuleDispatcherTest {
    ...
    private fun ruleRow(idempotencyKey: String?): OutboxRow = OutboxRow(
        ...
        payloadJson = moshi().adapter(CategoryRuleDeleteRequest::class.java)
            .toJson(CategoryRuleDeleteRequest(expectedRowVersion = 0L)),
    ...
    private fun dispatcherFor(stub: ApiService) = DeleteCategoryRuleDispatcher(
        apiProvider = { stub },
        payloadAdapter = moshi().adapter(CategoryRuleDeleteRequest::class.java),
    )
