...
fun ExpenseEditViewModel.loadFxReview(hasDraftChanges: Boolean) {
    ...
    if (state.fx.loading || state.expenseLoading || state.saving || state.itemsSaving || state.splitsSaving) return
    ...
    viewModelScope.launch {
        repository.fetchExpenseForFxReview(binding, expense.id).onSuccess { fresh ->
            _uiState.update {
                it.copy(expense = fresh, expenseLoading = false, fx = ExpenseFxUiState(task = fresh.fxTask))
            }
        }.onFailure { error ->
            _uiState.update {
