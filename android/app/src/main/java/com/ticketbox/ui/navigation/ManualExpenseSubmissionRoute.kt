...
internal fun ManualExpenseSubmissionRoute(
    ...
    var openedExpense by remember(clientRef, screenFactory) { mutableStateOf<Long?>(null) }
    val id = openedExpense
    if (id != null) {
        ExpenseEditRoute(id, screenFactory, onBack, onCompleted, onOpenRepaymentDrafts)
        return
    }
    ...
    SyncStatusScreen(vm, onBack, manualClientRef = clientRef, navigation = SyncStatusNavigation(
        onOpenExpense = { if (it > 0) openedExpense = it }, onOpenInbox = {}, onOpenBudget = {},
        onOpenRecurring = {}, onOpenGoalCreation = {}, onOpenGoalEdit = {}, onOpenRuleSubmission = {},
        onOpenIncomeSubmission = {}, onOpenRateSubmission = {}))
}
