...
interface IncomePlanActions {
    ...
    fun observeEdits(expectedBinding: LogicalSessionBinding): Flow<List<PendingIncomePlanEdit>>
    fun describeEdit(row: OutboxRow): PendingIncomePlanEdit?
    suspend fun recoverEdit(expectedBinding: LogicalSessionBinding, pending: PendingIncomePlanEdit, drop: Boolean): Result<Unit>
    ...
    suspend fun create(expectedBinding: LogicalSessionBinding, draft: IncomePlanDraft): Result<IncomePlan>
...
class IncomePlanRepository(
    ...
    private val incomePlanUpdateAdapter: JsonAdapter<IncomePlanEditPayload>,
) : IncomePlanActions {
    ...
    override fun describeEdit(row: OutboxRow): PendingIncomePlanEdit? {
        val binding = guard.captureLogicalBinding() ?: return null
        if (row.type != PendingMutationType.UpdateIncomePlan ||
            row.ownerKey != binding.ownerKey || row.ledgerId != binding.ledgerId
        ) return null
        return PendingIncomePlanEdit(row, incomePlanUpdateAdapter.readSupportedIncomeEdit(row.payloadJson))
    }

    override fun observeEdits(expectedBinding: LogicalSessionBinding): Flow<List<PendingIncomePlanEdit>> =
        outbox.observeActiveByTypes(setOf(PendingMutationType.UpdateIncomePlan), includeCompleted = true).map { rows ->
            if (guard.captureLogicalBinding() != expectedBinding) emptyList()
            else rows.mapNotNull(::describeEdit)
        }
    ...
    override suspend fun create(expectedBinding: LogicalSessionBinding, draft: IncomePlanDraft): Result<IncomePlan> =
        errors.safeCall {
            if (!canModifyLedger()) throw RepositoryException("当前角色为只读，无法修改账本。")
            guard.bindExact(expectedBinding).call { it.createIncomePlan(draft.toCreateRequest()).toDomain() }
        }
    ...
    override suspend fun enqueueUpdate(expectedBinding: LogicalSessionBinding, baseline: IncomePlan,
        ...
        val encoded = incomePlanUpdateAdapter.toJson(payload)
        if (incomePlanUpdateAdapter.readSupportedIncomeEdit(encoded) == null) throw RepositoryException("请检查计划月份和修改内容。")
        outbox.enqueue(boundRequest = bound, intent = PendingMutationIntent(
            type = PendingMutationType.UpdateIncomePlan,
            targetId = incomePlanTarget(baseline.publicId), payloadJson = encoded,
            expectedRowVersion = baseline.rowVersion, idempotencyKey = UUID.randomUUID().toString()))
    }
    ...
    override suspend fun archive(expectedBinding: LogicalSessionBinding, publicId: String,
        ...
        if (!canModifyLedger()) throw RepositoryException("当前角色为只读，无法修改账本。")
        guard.bindExact(expectedBinding).call {
