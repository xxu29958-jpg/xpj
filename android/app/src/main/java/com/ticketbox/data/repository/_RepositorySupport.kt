...
internal fun backendErrorUserMessage(errorCode: String, serverMessage: String): String {
    return backendErrorUserMessages[errorCode.trim()]
        ?: serverMessage.trim().ifBlank { "操作失败。" }
}

private val backendErrorUserMessages = mapOf(
    "invalid_token" to "绑定已失效，请重新绑定账本。",
    "legacy_auth_removed" to "请使用新版绑定方式。",
    ...
    "invalid_pairing_code" to "绑定码无效，请重新获取。",
    ...
    "permission_denied" to "当前角色为只读，无法修改账本。",
    "merchant_alias_not_found" to "商家别名不存在。",
    "merchant_alias_conflict" to "商家别名已指向其他商家。",
    "merchant_catalog_not_found" to "商家不存在或已删除。",
    // ADR-0043 tag management. tag_conflict steers the user to 合并 (契约 5);
    // tag_undo_not_found is the elapsed 5-minute undo window.
    "tag_not_found" to "标签不存在或已删除。",
    "tag_conflict" to "标签名已被占用，请改用合并。",
    ...
    "recurring_ledger_conflict" to "这条固定支出不属于当前账本，请重新打开记录。",
    "recurring_currency_conflict" to "原记录币种待确认，请核对固定支出。",
    "recurring_command_owner_unavailable" to "固定支出提交暂不可用，请重新打开应用。",
    ...
    "notification_source_invalid" to "通知来源暂不支持。",
    "server_error" to "暂时处理不了，请稍后再试。",
    "invalid_request" to "请求参数不正确。",
    ...
    "idempotency_key_required" to "操作未能完成，请重试。",
    "idempotency_key_in_progress" to "操作正在处理，请稍后再试。",
    "idempotency_key_reused" to "操作已处理，请勿重复提交。",
