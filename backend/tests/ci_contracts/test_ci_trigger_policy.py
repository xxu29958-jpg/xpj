...
workflow_parser = load_ci_script("ci_gap_workflow_parser.py")
...
def test_nvd_producer_paths_are_pinned(tmp_path: Path, monkeypatch) -> None:
    workflows = _github_workflows(tmp_path, monkeypatch)
    workflow = workflows / "nvd-database.yml"
    source = workflow.read_text(encoding="utf-8")
    workflow.write_text(
        source.replace('      - "android/build.gradle.kts"\n', "", 1),
        encoding="utf-8",
    )

    assert any("nvd-database.yml push paths" in item for item in _github_failures())
...
""",
    ...
    for loader in (workflow_yaml.load_workflow, workflow_parser._load_workflow):
        try:
            loader(workflow)
