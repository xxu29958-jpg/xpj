...
def captured_calendar(monkeypatch):
    rule = SimpleNamespace(timezone_name="UTC", revision=1)
    monkeypatch.setattr(insights_service, "current_calendar", lambda *a, **kw: rule)
    monkeypatch.setattr(recurring_service, "current_calendar", lambda *a, **kw: rule)
...
def test_candidate_groups_projected_money_instead_of_equal_unlabelled_integers(monkeypatch):
    ...
    expenses = [SimpleNamespace(merchant="Subscription", expense_time=when, confirmed_at=when, accounting_date=when.date(),
...
def test_candidate_missing_currency_cannot_be_reported_as_no_candidates(monkeypatch):
    ...
    expense = SimpleNamespace(merchant="Subscription", expense_time=when, confirmed_at=when, accounting_date=when.date(),
