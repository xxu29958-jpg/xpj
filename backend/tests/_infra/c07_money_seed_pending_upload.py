...
from tests._infra.c07_money_migration import seed_owner
...
def seed_legacy_pending_upload_money() -> tuple[int, int, int]:
    ...
    with SessionLocal() as db:
        ...
        db.add_all([pending_home, confirmed_home, pending_foreign])
        db.commit()
        return (
            int(pending_home.id),
            int(confirmed_home.id),
            int(pending_foreign.id),
        )
