...
def test_update_income_plan_partial(client: TestClient, *, identity) -> None:
    created = client.post(
        "/api/income-plans",
        headers={**negotiated_headers(client, identity.app_headers), "Idempotency-Key": str(uuid4())},
        json={"home_currency_code": "CNY", "intent_month": "2026-05", "label": "x", "source_type": "salary", "amount_cents": 100, "pay_day": 1},
    ).json()
    pid = created["public_id"]
...
def test_delete_income_plan_archives(client: TestClient, *, identity) -> None:
    created = client.post(
        "/api/income-plans",
        headers={**negotiated_headers(client, identity.app_headers), "Idempotency-Key": str(uuid4())},
        json={"home_currency_code": "CNY", "intent_month": "2026-05", "label": "tobearchived", "source_type": "salary", "amount_cents": 100, "pay_day": 1},
    ).json()
    pid = created["public_id"]
...
def test_restore_income_plan_reactivates(client: TestClient, *, identity) -> None:
    created = client.post(
        "/api/income-plans",
        headers={**negotiated_headers(client, identity.app_headers), "Idempotency-Key": str(uuid4())},
        json={"home_currency_code": "CNY", "intent_month": "2026-05", "label": "torestore", "source_type": "salary", "amount_cents": 100, "pay_day": 1},
    ).json()
    pid = created["public_id"]
...
def test_discretionary_subtracts_income_minus_fixed_minus_user_params(
    ...
    client.post(
        "/api/income-plans",
        headers={**negotiated_headers(client, identity.app_headers), "Idempotency-Key": str(uuid4())},
        json={"home_currency_code": "CNY", "intent_month": "2026-05", "label": "salary", "source_type": "salary", "amount_cents": 1_000_000, "pay_day": 1},
    )
    resp = client.get(
        "/api/budget/discretionary"
...
def test_discretionary_floors_at_zero_when_underwater(
    ...
    client.post(
        "/api/income-plans",
        headers={**negotiated_headers(client, identity.app_headers), "Idempotency-Key": str(uuid4())},
        json={"home_currency_code": "CNY", "intent_month": "2026-05", "label": "salary", "source_type": "salary", "amount_cents": 100_000, "pay_day": 1},
    )
    resp = client.get(
