"""RED/GREEN contract: a blocked /web auth seam must not starve /api/health.

This is a behavior test against a real Uvicorn accept/event loop, not a
source-text check for ``run_in_threadpool`` / helper names.
"""
...
from app.runtime_isolation import reset_auth_offload_limiter
...
class _BlockingSession:
    """Stand-in for ``SessionLocal()`` that occupies the calling thread."""
...
def test_blocked_web_session_db_does_not_starve_independent_health(
    ...
    """A blocked browser-session DB seam must leave /api/health independently schedulable."""
    ...
    opener = _opener()
    with _running_server(_isolation_app()) as origin:
        ...
        with opener.open(urllib.request.Request(f"{origin}/api/health"), timeout=_HEALTH_BUDGET_SECONDS) as health:
...
def test_threadpool_saturation_does_not_deadlock_health(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    reset_auth_offload_limiter(1)
    entered = threading.Event()
    release = threading.Event()
    monkeypatch.setattr(
        "app.middleware.web_session.SessionLocal",
    ...
    try:
        with _running_server(_isolation_app()) as origin:
            ...
            first = threading.Thread(target=blocked_auth, daemon=True)
            first.start()
            assert entered.wait(timeout=2)
            second = threading.Thread(target=blocked_auth, daemon=True)
            second.start()
            started = time.monotonic()
            with opener.open(
