...
@pytest.mark.parametrize(
    ("sync_times", "timezone"),
    [("invalid", "UTC"), ("09:10", "Unavailable/Timezone")],
    ids=["invalid-quote-times", "unavailable-quote-timezone"],
)
def test_invalid_quote_schedule_still_continues_accepted_bills_on_the_existing_thread(
    monkeypatch: pytest.MonkeyPatch, ticks: _ScheduledTicks, sync_times: str, timezone: str,
) -> None:
    settings = SimpleNamespace(
        fx_rate_auto_sync_enabled=True, fx_rate_sync_times=sync_times, fx_rate_sync_timezone=timezone,
    )
    ticks.count = 2
    refill = Mock(side_effect=[32, 64, 0])
    scheduled_quote = Mock()
    monkeypatch.setattr(scheduler, "get_settings", lambda: settings)
    monkeypatch.setattr(scheduler, "threading", SimpleNamespace(Event=lambda: ticks, Thread=threading.Thread))
    monkeypatch.setattr(scheduler, "refill_pending_expense_fx", refill)
    monkeypatch.setattr(scheduler, "_run_scheduled_fx_sync", scheduled_quote)
    monkeypatch.setattr(scheduler._runtime, "counters", scheduler.FxRateSyncStatus())

    worker = scheduler.start_fx_rate_scheduler()
    try:
        assert worker is not None, "Invalid daily quote configuration must not strand accepted bill continuation"
        worker.thread.join(timeout=1)
        assert not worker.thread.is_alive()
        assert worker.thread.name == "fx-rate-scheduler" and worker.thread.daemon
    finally:
        if worker is not None:
            worker.stop()

    assert refill.call_args_list == [call(after_id=0), call(after_id=32), call(after_id=64)]
    assert ticks.delays == [30, 30, 30]
    assert ticks.current.time() == time(9, 10, 30)
    scheduled_quote.assert_not_called()
    status = scheduler.fx_rate_sync_status()
    assert status.scheduler_config_error is True
    assert (status.success_count, status.failed_count, status.last_error, status.last_success_at) == (0, 0, None, None)
