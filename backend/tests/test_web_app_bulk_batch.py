...
from _web_bulk_test_support import (
    assert_native_checkboxes_are_outside_links as _assert_native_checkboxes_are_outside_links,
)
from _web_bulk_test_support import (
...
def test_web_pending_bulk_selection_markup_and_js_field_name(web_client: TestClient, *, identity) -> None:
    ...
    assert 'class="exp-row-detail"' in resp.text
    _assert_native_checkboxes_are_outside_links(resp.text)
    ...
    assert 'row.setAttribute("aria-current", "true");' in drawer_js
    assert "markCurrent(null);" in drawer_js
    assert 'candidate.removeAttribute("aria-current");' in drawer_js
    assert 'row.getAttribute("aria-disabled") === "true"' in drawer_js
    assert '".exp-row-detail[data-fragment-url]"' in drawer_js
    assert 'getAttribute("aria-current") === "true"' in hotkeys_js
    assert "aria-selected" not in drawer_js
    assert "aria-selected" not in hotkeys_js
