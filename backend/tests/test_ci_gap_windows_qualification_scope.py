...
def _write_scope(workflow: Path, *, include_installer: bool) -> None:
    installer = (
...
def test_installer_dependencies_are_part_of_github_pr_scope(tmp_path: Path) -> None:
    ...
    _write_scope(workflows / "ci.yml", include_installer=True)
...
def test_installer_dependency_scope_rejects_exe_only_classification(
    ...
    mod = load_ci_gap_module("ci_gap_windows_qualification")
    workflows = tmp_path / ".github" / "workflows"
    _write_scope(workflows / "ci.yml", include_installer=False)

    assert mod.github_pr_installer_dependency_scope_violations([workflows]) == [
        "GitHub PR scope must run the installer for backend provenance scripts",
        "GitHub PR scope must run the installer for Desktop Manager provenance script",
        "GitHub PR scope must run the installer for Alembic runtime inputs",
    ]
