...
def test_valid_desktop_bridge_projects_bound_viewer_and_only_its_ledger(
    ...
    assert 'data-theme-sync="server"' in response.text
    assert 'action="/web/auth/logout"' not in response.text
    assert "退出此浏览器" not in response.text
