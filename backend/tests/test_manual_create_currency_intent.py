...
def test_new_manual_create_retains_captured_jpy_home_under_current_cny(create_context):
    payload = ExpenseManualCreateRequest(amount_cents=12, home_currency_code="JPY", category="餐饮")
    ...
    create_context.commit.assert_called_once()


def test_legacy_explicit_original_money_can_still_be_accepted_without_rewriting_its_body(create_context):
    payload = ExpenseManualCreateRequest(original_currency="CNY", original_amount="12.00", category="餐饮")
    result = _insert(create_context, payload)
...
def test_new_bare_minor_amount_without_currency_is_refused_before_any_fact_write(create_context):
    with pytest.raises(AppError) as error:
        _insert(create_context, ExpenseManualCreateRequest(amount_cents=12, category="餐饮"))
...
def test_accepted_legacy_bare_amount_replays_the_existing_fact_before_new_write_validation(create_context, monkeypatch):
    payload = ExpenseManualCreateRequest(amount_cents=12, client_ref="old-ref", category="餐饮")
    accepted = SimpleNamespace(draft_request_fingerprint=owner._manual_request_fingerprint(payload))
    ...
    assert owner.create_manual_expense(create_context, payload, auth) is accepted
    create_context.add.assert_not_called()
    create_context.commit.assert_not_called()
