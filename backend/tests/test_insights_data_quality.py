...
def _patch(client: TestClient, expense_id: int, **fields) -> None:
    response = client.patch(
        f"/api/expenses/{expense_id}", headers=app_headers(), json=fields
    )
    assert response.status_code == 200, response.text


def test_data_quality_empty(client: TestClient) -> None:
...
def test_data_quality_missing_amount_and_merchant(client: TestClient) -> None:
    # Upload creates a pending row with no amount and no merchant.
    upload_png(client)
    # Second row: fill amount but leave merchant blank.
    eid_b = upload_png(client)
    _patch(client, eid_b, amount_cents=1234)
    # Third row: fill both (ready).
    eid_c = upload_png(client)
    _patch(client, eid_c, amount_cents=2222, merchant="星巴克")
...
def test_data_quality_missing_category_counts_pending_and_confirmed(
    ...
    eid = upload_png(client)
    _patch(client, eid, amount_cents=500, merchant="A", category="未分类")
...
def test_data_quality_suspected_duplicates(client: TestClient) -> None:
    # Two uploads with the same amount + merchant trigger duplicate detection.
    eid_a = upload_png(client)
    _patch(client, eid_a, amount_cents=4500, merchant="美团外卖")
    eid_b = upload_png(client)
    _patch(client, eid_b, amount_cents=4500, merchant="美团外卖")
...
def test_data_quality_ledger_isolation(client: TestClient) -> None:
