...
def test_foreign_home_amount_drives_stats_reports_budget_and_export(
    ...
    _set_manual_rate(client, currency_code="USD", rate_date="2026-05-04", rate_to_cny="7")
...
def test_missing_foreign_rate_stays_pending_without_fake_home_amount(
    ...
    confirm = client.post(f"/api/expenses/{payload['id']}/confirm", headers=app_headers())
...
def test_resolved_foreign_snapshot_does_not_drift_after_rate_update(
    ...
    _set_manual_rate(client, currency_code="USD", rate_date="2026-05-04", rate_to_cny="8")

    frozen = client.get(f"/api/expenses/{first['id']}", headers=app_headers())
...
def test_csv_import_accepts_old_home_rows_and_resolves_new_foreign_rows_with_backend_rate(
    ...
    rows = client.get(f"/api/imports/csv/{public_id}/rows", headers=app_headers())
