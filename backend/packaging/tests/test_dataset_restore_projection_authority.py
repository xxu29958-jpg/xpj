...
def test_runtime_environment_acl_contract_is_exact_and_writer_owned() -> None:
    ...
    assert "-BackendServiceName ([string]$ProjectionContract.backend_service_name)" in (
        projection_writer
    )
...
"""
    ...
    artifact_contract = RESTORE_ARTIFACTS.read_text(encoding="utf-8-sig")
