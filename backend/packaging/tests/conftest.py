...
def _scheduler_group_violation(
    item: pytest.Item,
    resource: str,
) -> str | None:
    groups = tuple(
...
def pytest_collection_modifyitems(
    ...
    for item in items:
        if item.get_closest_marker("xdist_group") is not None:
            raise pytest.UsageError(
                f"{item.nodeid} must not author the generated xdist_group marker"
            )
    ...
    for item, resource in resource_assignments:
        group_violation = _scheduler_group_violation(item, resource)
        if group_violation:
            raise pytest.UsageError(group_violation)
    if not _strict_runtime_enabled():
        return
