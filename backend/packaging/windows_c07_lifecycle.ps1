...
<#
.SYNOPSIS
  Host-authoritative lifecycle artifacts for ADR-0073 C07 BIGINT expansion.
.DESCRIPTION
  The lifecycle lock, protected installation identity, live PostgreSQL
  identity, writer-stop observations, and typed protected evidence artifacts
  jointly authorize a monotonic C07 operation.

  Dot-source windows_installation_safety.ps1, windows_lifecycle_lock.ps1,
  windows_service_lifecycle.ps1, and windows_c07_database.ps1 before calling
  these functions.  The PostgreSQL superuser SecureString remains process-local
...
function Read-TicketboxC07AuthorityCore {
    ...
    if ($stage -eq "ready") {
        Read-TicketboxC07ReadyVerification $result | Out-Null
    }
    return $result
}
