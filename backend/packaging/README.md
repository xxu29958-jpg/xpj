...
- PostgreSQL 活体宿主权威统一归属 `windows_database_safety.ps1`：解析器只接受当前安装目录内的精确 `pg_ctl runservice` SCM 合同，并从 `postmaster.pid` 复核 PID、端口与数据目录。legacy 物理 `DataRoot\pgdata` 仍执行整条祖先 no-reparse 校验；新 runtime 路径只在共享安装安全层两次验证专用 junction、Volume GUID target、v2 marker、ACL 与 PG/backend 服务 SID 后接受，不能把任意 reparse 当成受管路径。C07 仅保留旧函数名与 `ticketbox-c07-host-db-authority-v1` schema 的兼容适配，不再拥有 Windows 路径/SCM 判定；失败统一投影为 `postgres_host_authority_validation_failed` / `TBX-INSTALL-POSTGRES-HOST`。
...
- 面向 Inno UI/支持人员的 `ticketbox-install-public-failure-v2` 是 Windows 安装通用投影，固定包含 support code、lifecycle stage、operation/installation identity、retry class、database mutation state 与 protected/public log 路径；它不依赖 C07 receipt 才能发布。`windows_c07_failure_summary.ps1` 只在既有 C07 schema lifecycle receipt 存在时追加兼容审计投影，不能替代、改写或阻止通用 public receipt，也不再承载 installation-owner、SCM 或路径裁决。
...
## 审计/开发兼容工具：本机 PostgreSQL 一键安装（legacy）

`install_ticketbox.ps1` 是旧用户和源码开发机的兼容工具，不进入 Inno EXE，也不是正式 Windows 分发、恢复或身份生命周期的权威实现。它仍保留旧式长期凭据文件行为，仅用于审计既有兼容面；不得据此放宽正式安装器合同，也不得把它的旧 artifact 提升为当前安装状态。
...
Docker/WSL/PS7。该 legacy 工具仍假设本机已装 PostgreSQL 服务，只供老用户/开发机临时兼容；面向用户的 Windows 分发只认上面的 Inno 捆绑安装器流程。
