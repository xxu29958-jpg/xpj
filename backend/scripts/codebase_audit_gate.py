...
STRICT_EQUALITY_BASELINE.update({
    "backend_pytest_count": 2345,
    "mutate_token_carriers": 77,
    "mutate_token_exempted": 122,
    "mutate_token_reason_create_row": 32,
})  # +2 ADR-0053 web merchant catalog hide/delete OCC carriers; +1 create-row exemption for POST /web/merchants/catalog/create; +3 collected pytest delta for viewer write-guard parameter cases; +1 ADR-0054 merchant catalog key-changing rename live-config blocker test; +1 ADR-0054 merchant catalog merge mutate-token carrier; +7 collected pytest delta for merge contract and 401 route matrix. +2 ADR-0054 web merchant catalog rename/merge OCC carriers; +4 collected pytest delta for web merge UX and route inventory.
