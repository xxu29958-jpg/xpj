...
CODEBASE_DEBT_LIMITS: DebtCounts = {
    ...
    "generic_raises": 6,  # −3 PG-only slice 5 (retired v1_migration handler RuntimeError raises + mark_v1_cut_over). +1 P1 启动迁移属主预检 RuntimeError(_assert_role_can_alter_existing_schema 致命启动条件,无 HTTP AppError 适配,镜像 main.py 既有启动 RuntimeError 守卫). +1 P1 启动迁移前备份 fail-closed RuntimeError(_backup_before_upgrade 备份失败即致命中止,镜像同款启动 RuntimeError)
    "todo_markers": 9,
