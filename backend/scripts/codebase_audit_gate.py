...
STRICT_EQUALITY_BASELINE.update({"mutate_token_carriers": 61, "backend_pytest_count": 2254})  # +1 /web pending batch-undo token carrier; +3 tests. +1 finding-三摊 Slice B ck_expenses_row_version_positive CHECK round-trip (test_alembic_expense_row_version_check_migration). +3 issue #65 slice A owner device delete (test_my_devices: delete-revoked→204+gone / delete-current→409 / delete-active→409; 401/403/404 fold into the existing require-auth/viewer/wrong-ledger tests).
