...
CODEBASE_DEBT_LIMITS: DebtCounts = {
    ...
    # silently re-creep.
    "unreferenced_modules": 208,  # +2 ADR-0043 tag_management_service + tag_undo_service (covered via /api/tags route tests, not direct import); +2 slice C web_tags + owner_console._tag_cleanup (HTTP-tested, not direct import); −15 PG-only slice 2 (retired _migrations/ + _validate/ submodules); −1 slice 5 (retired cut-over modules); −4 ratchet tightens on the gate's own improved-INFO (2026-06-11, 2026-06-12 ×3 — 批14 retiring web_stats, 扫尾W2 list_sent_for_expense wired); +6 ADR-0049 slice 1 Debt domain (models.debt / routes.debts / schemas._debts / debt_service._create+_query+_fold — covered via /api/debts route tests + test_debt_fold service imports, not direct submodule-path import)
    "import_cycles": 0,
