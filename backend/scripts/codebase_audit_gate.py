...
CODEBASE_DEBT_LIMITS: DebtCounts = {
    ...
    # silently re-creep.
    "unreferenced_modules": 203,  # +2 ADR-0043 tag_management_service + tag_undo_service (covered via /api/tags route tests, not direct import); +2 slice C web_tags + owner_console._tag_cleanup (HTTP-tested, not direct import); −15 PG-only slice 2 (retired _migrations/ + _validate/ submodules); −1 slice 5 (retired cut-over modules); −3 ratchet tightens on the gate's own improved-INFO (2026-06-11, 2026-06-12 ×2 — the second from 批14 retiring web_stats)
    "import_cycles": 0,
