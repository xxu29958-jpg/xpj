...
CODEBASE_DEBT_LIMITS: DebtCounts = {
    ...
    "files_over_500": 16,  # +2 ADR-0049 slice 3: routes/debts.py (580 — the 5 new member-repayment-proposal routes on the cohesive debt route module) + tests/test_member_repayment_proposal.py (the comprehensive §11 proposal suite). Splitting either purely to clear the line is the wrong trade for this slice; the test file is a per-concern split candidate for a follow-up.
