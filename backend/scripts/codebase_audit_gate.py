...
STRICT_EQUALITY_BASELINE["backend_pytest_count"] = 1854  # +1 ADR-0049 slice 3 paid_at FX-date fingerprint regression; +6 ADR-0049 slice 4 bill_split→Debt linkage (test_bill_split_debt_linkage: accept-creates-member-Debt rollout-on / no-Debt rollout-off / re-accept-no-duplicate / reject-no-Debt / foreign-parent-stays-home-shape / debt-failure-rolls-back-whole-accept[real_db]); +8 ADR-0049 slice 5 account-scoped cross-ledger participant confirm/reject §5.2 (test_member_repayment_proposal_cross_ledger ×6: non-member creditor confirms+clears with shell-redacted ledger_id / non-member creditor rejects / non-member creditor lists pending proposal / member creditor in shared ledger settles keeping ledger_id / non-participant existence-hiding 404 on read+list+confirm / cross-ledger confirm idempotent replay; test_bill_split_debt_linkage ×2: two-sessions concurrent accept creates single Debt[real_db] / duplicate uq_debts_source rejected); +26 ADR-0049 slice 6 debt_repayment goal §6 (test_debt_repayment_goal ×11 evaluator: in_progress / all-cleared-achieves+latches-once / one-open / voided-not_evaluable+review / replace-bumps-version+freezes-old-set / unlink-open-achieves-new-version-not-old / achieved-then-reopen-stays-achieved / viewer-read-computes-achieved-without-latching / not_evaluable-recovers-via-link-replace / debt-vs-spending-list-isolation / spending-limit-regression; test_debt_repayment_goal_lifecycle ×14 guards: replace requires-auth-401 / idempotent-replay / stale-409 / requires-idem-key-422 / unknown-debt-404 / reject-spend-fields / requires-≥1-debt / unknown-debt-404-create / cross-ledger-isolation / viewer-403-create+replace / patch-rejects-debt-goal / two-active-debt-goals-coexist / replace-rejects-spending-goal / archive-hides; test_alembic_goal_debt_repayment_migration ×1 widen-goals round-trip[real_db] incl. scope-index goal_type predicate assertions) — 6-lens adversarial review fixed P1 archive_goal debt dispatch (int(None) 500) + P2/P3 coverage (two-goals-coexist, viewer-read-no-latch, not_evaluable-recovery, replace-rejects-spending, debt-links 401, migration scope-index predicate)
