...
_ADR_0049_EXEMPTED_GRANDFATHER = (119, 120)  # issue #65 slice A re-points the single in-flight DOWN-ratchet up-hop 119->120 (+1 owner "My Devices" delete route, account-owner twin of the loopback /owner device delete — no row_version → exempt). The name is historical (first used for ADR-0049); it is the generic single in-flight exemption-add hop. Prior 116->119 hop is dead history (main sits at 119).
