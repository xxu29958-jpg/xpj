...
STRICT_EQUALITY_BASELINE: DebtCounts = {
    ...
    "mutate_token_exempted": 119,  # issue #65 slice 6a: +3 owner-facing "My Devices" mutating routes (POST /api/ledgers/{ledger_id}/devices/{public_id}/rename admin_single_writer + .../revoke terminal_flag_flip + .../pairing-codes create_row; account-owner-scoped twins of the loopback /api/admin/devices entries, no row_version → exempt). DOWN-ratchet single in-flight up-hop re-pointed 116 -> 119 (prior 115 -> 116 = dead history, main was at 116).
...
STRICT_EQUALITY_BASELINE.update({"mutate_token_carriers": 61, "backend_pytest_count": 2251})  # +1 /web pending batch-undo token carrier; +3 tests. +1 finding-三摊 Slice B ck_expenses_row_version_positive CHECK round-trip (test_alembic_expense_row_version_check_migration).
...
_ADR_0049_EXEMPTED_GRANDFATHER = (116, 119)  # issue #65 slice 6a re-points the single in-flight DOWN-ratchet up-hop 116->119 (+3 owner "My Devices" mutating routes, all account-owner twins of existing loopback admin device routes — no row_version → exempt). The name is historical (first used for ADR-0049); it is the generic single in-flight exemption-add hop. Prior 115->116 hop is dead history (main sits at 116).
