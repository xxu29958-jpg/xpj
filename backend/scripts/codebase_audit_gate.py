...
STRICT_EQUALITY_BASELINE: DebtCounts = {
    ...
    "mutate_token_carriers": 53,  # +4 ADR-0049 slice 2 fold-changing facts carry expected_row_version: POST /api/debts/{id}/repayments|adjustments|repayment-voids|void (§2.1 stale-intent fence + §3.6 fingerprint)
    "mutate_token_exempted": 110,  # −2 PG-only slice 5 (retired /owner/migration-readiness cut-over + pre-v1-backup exemptions); +1 ADR-0049 slice 1 POST /api/debts create
    "mutate_token_reason_admin_single_writer": 9,
    "mutate_token_reason_append_only_fact": 4,
    "mutate_token_reason_batch_db_write": 19,
    "mutate_token_reason_create_row": 27,  # +1 ADR-0049 slice 1 POST /api/debts
...
def _compute_ratchet_findings(
    ...
    for key in sorted(STRICT_EQUALITY_BASELINE):
        ...
        debt_create_0049 = (key, base_val, current_val) == ("mutate_token_exempted", 109, 110)
        ...
        elif key in BASELINE_RATCHET_DOWN and current_val > base_val and not debt_create_0049:
            movement_violations.append(
