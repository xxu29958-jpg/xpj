...
STRICT_EQUALITY_BASELINE.update({"mutate_token_carriers": 68, "backend_pytest_count": 2313})  # +5 ADR-0052 Slice 2 actual PR-delta: monthly budget archive DELETE plus recycle-bin restore token carriers in live OpenAPI; +35 collected pytest delta from budget archive/restore tests, route inventory, and API restore auth coverage.
