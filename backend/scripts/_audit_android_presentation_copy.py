...
ALLOWLIST: dict[str, str] = {
    ...
    "android/app/src/main/java/com/ticketbox/ui/appearance/BackgroundCatalog.kt :: 冷白纸面与低对比层次，长时间整理账单也不刺眼。": "static catalog: built-in background description",
    ...
    "android/app/src/main/java/com/ticketbox/ui/appearance/BackgroundCatalog.kt :: 深色纸面与低饱和绿，适合夜间核对账单。": "static catalog: built-in background description",
    ...
    r"android/app/src/main/java/com/ticketbox/ui/screens/pending/PendingScreenModels.kt :: ^(?:\d{4}\s*[-/.年]\s*)?\d{1,2}\s*[-/.月]\s*\d{1,2}\s*日?(?:\s+周[一二三四五六日天])?(?:\s+\d{1,2}\s*[:：]\s*\d{2}(?:\s*[:：]\s*\d{2})?)?$": "internal keyword-match: parser-only date-noise regex, never rendered",
