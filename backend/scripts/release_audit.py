...
_REQUIRED_LANE_FILES = frozenset(
    {
        "_audit_ci_gap.py",
...
def _missing_required_lanes(lanes: list[tuple[str, str]]) -> list[str]:
    return sorted(
        _REQUIRED_LANE_FILES - {filename for _label, filename in lanes}
    )
...
def main() -> int:
    ...
    if not lanes:
        ...
        return 1
    missing_required = _missing_required_lanes(lanes)
    if missing_required:
        print(
            "RELEASE AUDIT: required product-risk lane(s) missing: "
