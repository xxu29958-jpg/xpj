...
class DatabaseMaintenanceContractError(RuntimeError):
    """The frozen maintenance process is outside its closed runtime contract."""
...
def resolve_generation_program(path: Path) -> Path:
    if path != Path(GENERATION_PROGRAM_FILENAME):
        raise DatabaseMaintenanceContractError("generation program must be the payload-root artifact")
...
def _maintenance_source_path(filename: str) -> Path:
    if getattr(sys, "frozen", False):
        ...
        if not isinstance(bundle_root, str) or not bundle_root:
            raise DatabaseMaintenanceContractError("frozen database maintenance source root is unavailable")
...
def _temporary_database_package(source_path: Path) -> Iterator[None]:
    ...
    if database_module_name in sys.modules:
        raise DatabaseMaintenanceContractError("standalone database maintenance process already loaded app.database")
    app_package = importlib.import_module("app")
    if hasattr(app_package, "database"):
        raise DatabaseMaintenanceContractError(
    ...
    try:
        ...
        if sys.modules.get(database_module_name) is not package:
            raise DatabaseMaintenanceContractError("standalone database maintenance package identity changed")
    finally:
...
def load_standalone_database_module(
    *,
    ...
    if not source_path.is_file():
        raise DatabaseMaintenanceContractError("standalone database maintenance source is unavailable")
    ...
    if isinstance(existing, ModuleType):
        if Path(str(existing.__file__)).resolve() != source_path:
            raise DatabaseMaintenanceContractError("standalone database maintenance module identity changed")
        return existing
    spec = importlib.util.spec_from_file_location(module_name, source_path)
    if spec is None or spec.loader is None:
        raise DatabaseMaintenanceContractError("standalone database maintenance module is unavailable")
...
def assert_maintenance_libpq_environment(pgpassfile: Path) -> None:
    ...
    if len(pg_entries) != 1 or pg_entries[0][0].upper() != "PGPASSFILE":
        raise DatabaseMaintenanceContractError("database maintenance helper libpq environment is not sealed")
    try:
        ...
        raise DatabaseMaintenanceContractError("database maintenance helper libpq environment is not sealed") from exc
    if actual != expected:
        raise DatabaseMaintenanceContractError("database maintenance helper libpq environment is not sealed")
