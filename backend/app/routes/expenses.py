...
def get_pending_expenses(
    ...
    return list_pending(db, tenant.id)
...
def post_manual_expense(
    ...
    return create_manual_expense(db, payload, tenant.id)
...
def get_confirmed_expenses(
    ...
    items, total = list_confirmed(db, tenant_id=tenant.id, page=page, page_size=page_size, month=month, category=category)
...
def get_expense_categories(
    ...
    return CategoriesResponse(items=list_categories(db, tenant.id))
...
def get_expense_months(
    ...
    return MonthsResponse(items=list_months(db, tenant.id))
...
def get_expenses_csv(
    ...
    content = "\ufeff" + export_confirmed_csv(db, tenant_id=tenant.id, month=month, category=category)
...
def get_expense_detail(
    ...
    return get_expense(db, expense_id, tenant.id)
...
def get_expense_image(
    ...
    expense = get_expense(db, expense_id, tenant.id)
...
def get_expense_thumbnail(
    ...
    path, media_type = ensure_thumbnail_file(db, expense_id, tenant.id)
    return FileResponse(path=path, media_type=media_type)
...
def patch_expense(
    ...
    return update_expense(db, expense_id, tenant.id, payload)
...
def post_confirm_expense(
    ...
    return confirm_expense(db, expense_id, tenant.id)
...
def post_reject_expense(
    ...
    return reject_expense(db, expense_id, tenant.id)
...
def post_retry_ocr(
    ...
    return retry_expense_ocr(db, expense_id, tenant.id)
...
def post_recognize_text(
    ...
    return recognize_expense_text(db, expense_id, tenant.id, payload)
...
def post_mark_not_duplicate(
    ...
    return mark_expense_not_duplicate(db, expense_id, tenant.id)
