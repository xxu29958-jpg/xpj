"""'本月可自由支配' formula.
    ...
    discretionary = income - fixed - savings_target - reserved_buffer

Capped at zero (negative means the user is already underwater on
fixed + planned outflows; v1.1 UI surfaces that as a separate alert).
...
class DiscretionaryBreakdown:
    ...
    step ("月收入 X - 固定支出 Y - 储蓄目标 Z = 可自由支配 W") without
    each surface duplicating the maths.
    """
