...
def _publish_windows_file_no_replace(
    source: Path,
    destination: Path,
    *,
    owner_sid: str,
...
def write_protected_file_no_replace(
    path: Path,
    text: str,
    *,
    service_owned: bool,
) -> None:
    """Durably stage and publish one new authority without replacement."""

    if not path.is_absolute() or not path.name:
        raise ValueError("protected publication path must be an absolute file path")
    staging = path.parent / f".{path.name}.{uuid4()}.staging"
    primary: BaseException | None = None
    cleanup: list[BaseException] = []
    try:
        if os.name == "nt":
            ...
            write_protected_file_exclusive(staging, text)
            with hold_protected_file_for_read(staging):
    ...
    if primary is not None and cleanup:
        raise BaseExceptionGroup(
            "protected no-replace publication and cleanup failed",
            [primary, *cleanup],
        ) from primary
    if primary is not None:
        raise primary
