...
def recurring_merchant_fits_storage_shape(*, merchant_name: str, merchant_key: str) -> bool:
    return (
        len(merchant_name) <= RECURRING_MERCHANT_MAX_LENGTH
        and len(merchant_key) <= RECURRING_MERCHANT_MAX_LENGTH
    )
...
def ensure_recurring_merchant_storage_shape(*, merchant_name: str, merchant_key: str) -> None:
    ...
    if not recurring_merchant_fits_storage_shape(
