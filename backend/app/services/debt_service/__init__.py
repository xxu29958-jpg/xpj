...
``_money`` / ``_guards`` submodules. The submodules exist only to keep each file
under the 500-line layering budget and to pin §2.1 in one place.
...
``void_debt`` — each serialized on the parent Debt row per §2.1.
"""
