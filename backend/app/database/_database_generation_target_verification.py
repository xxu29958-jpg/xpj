...
RESULT_SCHEMA = "ticketbox-database-generation-target-verification-v1"
LIVE_DATABASE = "ticketbox"
RESTORE_PREFIX = "ticketbox_c07_restore_"
    ...
    try:
        with (
            ...
            with (
                ...
                _prearmed_transaction(connection, timeout_ms=_TIMEOUT_MS),
            ):
                _assert_migrator_authority(connection, database=database)
                revision = _current_revision(connection)
                if revision != target_revision:
                    raise DatabaseGenerationTargetVerificationError(
                        "target verification database is not at the requested revision"
                    )
                return revision, _target_shape_sha256(connection), _money_facts(connection)
    finally:
