...
from backend_manager.product_data import PRODUCT_WORKSPACES, ProductDataError
...
_PRODUCT_COMMAND_PATTERN = re.compile(r"/api/product/inbox/expenses/([^/]+)/commands\Z")
_MAX_PRODUCT_COMMAND_BYTES = 64 * 1024
_MAX_PRODUCT_PAIR_BYTES = 1024
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def end_headers(self) -> None:
        ...
        policy = (
            _PRODUCT_CONTENT_SECURITY_POLICY
            if path in {"/app", "/app.html", *_PRODUCT_ASSET_TYPES}
    ...
    def _serve_product_workspace(self, parsed_path: urllib.parse.SplitResult) -> None:
        srv: ControlServer = self.server  # type: ignore[assignment]
        if not self._token_allowed():
            self._send(403, b"forbidden", "text/plain; charset=utf-8")
            return
        workspace = parsed_path.path.removeprefix("/api/product/")
        if workspace not in PRODUCT_WORKSPACES:
            self._send_json({"error": "workspace_not_found"}, code=404)
            return
        try:
            ...
            return
        ledger_values = query.get("ledger_id")
        if set(query) - {"ledger_id"} or (ledger_values is not None and len(ledger_values) != 1):
            self._send_json({"error": "invalid_request"}, code=400)
            return
        ledger_id = ledger_values[0] if ledger_values else None
        try:
            payload = srv.controller.product_workspace(workspace, ledger_id)
        except ProductDataError as exc:
            self._send_json(
    ...
    def _serve_product_inbox_command(
        self,
        parsed_path: urllib.parse.SplitResult,
        match: re.Match[str],
    ) -> None:
        srv: ControlServer = self.server  # type: ignore[assignment]
        ...
        if set(query) - {"ledger_id"} or (ledger_values is not None and len(ledger_values) != 1):
            self._send_json({"error": "invalid_request"}, code=400)
            return
        public_id = urllib.parse.unquote(match.group(1))
        ...
        if not idempotency_key or len(idempotency_key) > 128:
            ...
            return
        payload = self._read_json_body(max_bytes=_MAX_PRODUCT_COMMAND_BYTES)
        if payload is None:
            self._send_json({"error": "invalid_request"}, code=400)
            return
        ledger_id = ledger_values[0] if ledger_values else None
        try:
            response = srv.controller.product_inbox_command(
                public_id,
                ledger_id,
                payload,
                idempotency_key,
    ...
    def _serve_product_asset(self, path: str) -> None:
        srv: ControlServer = self.server  # type: ignore[assignment]
        asset = srv.product_css if path == "/product.css" else srv.product_js
        try:
            body = asset.read_bytes()
        except OSError:
            self._send(404, b"not found", "text/plain; charset=utf-8")
            return
        self._send(200, body, _PRODUCT_ASSET_TYPES[path])
    ...
    def _serve_product_pair(self) -> None:
        ...
        if not self._authorized():
            self._send(403, b"forbidden", "text/plain; charset=utf-8")
            return
        payload = self._read_json_body(max_bytes=_MAX_PRODUCT_PAIR_BYTES)
    ...
    def do_GET(self) -> None:  # noqa: C901 - explicit fail-closed route matrix
        ...
        elif parsed_path.path in _PRODUCT_ASSET_TYPES and not parsed_path.query:
            self._serve_product_asset(parsed_path.path)
        ...
        elif parsed_path.path.startswith("/api/product/"):
            self._serve_product_workspace(parsed_path)
    ...
    def do_POST(self) -> None:  # noqa: C901 - explicit fail-closed route matrix
        ...
        if parsed_path.path == "/api/product/unpair" and not parsed_path.query:
            ...
            return
        product_match = _PRODUCT_COMMAND_PATTERN.fullmatch(parsed_path.path)
        if product_match is not None:
            self._serve_product_inbox_command(parsed_path, product_match)
            return
...
class ControlServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        *,
        ...
        instance_secret: str,
        ui_html: Path,
        product_html: Path | None = None,
        product_css: Path | None = None,
        product_js: Path | None = None,
        ...
        self.product_html = product_html or ui_html
        self.product_css = product_css or self.product_html.with_name("product.css")
        self.product_js = product_js or self.product_html.with_name("product.js")
