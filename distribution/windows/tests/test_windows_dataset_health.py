...
from ticketbox_lifecycle.policy.health_attestation import sign_challenge
...
def _request(tmp_path: Path):
    return replace(
        ...
        health_attestation_key=_HEALTH_ATTESTATION_KEY,
    )
...
def _stub_health_body(monkeypatch, body: bytes, *, attestation: str | None = None) -> None:
    monkeypatch.setattr(windows_dataset, "new_challenge", lambda: _HEALTH_CHALLENGE)
    monkeypatch.setattr(
        windows_dataset,
        "fetch_installation_health",
        lambda _port, *, challenge: (
            (200, body, attestation)
            if challenge == _HEALTH_CHALLENGE
            else (400, b"", None)
        ),
    )


def _stub_json_health(monkeypatch, payload: object, *, attested: bool = True) -> None:
    _stub_health_body(
        monkeypatch,
        json.dumps(payload).encode("utf-8"),
        attestation=(
            sign_challenge(_HEALTH_ATTESTATION_KEY, _HEALTH_CHALLENGE)
            if attested
            else None
        ),
    )
...
def test_health_rejects_static_identity_without_backend_attestation(
    tmp_path: Path,
    monkeypatch,
) -> None:
    request = _request(tmp_path)
    ...
    _stub_json_health(monkeypatch, _health_payload(request), attested=False)
    monkeypatch.setattr(adapter, "_live_dataset_id", lambda _request: request.dataset_id)
